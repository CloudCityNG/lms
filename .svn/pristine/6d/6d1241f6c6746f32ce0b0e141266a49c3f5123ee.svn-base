...
$occupation_id  = trim(getgpc('occupation_id','G'));
$USERID=$_SESSION['_user']['user_id'];
 
if($USERID){
    if($device_id && $occupation_id){
        $sql="select * from `labs_run_devices` where `course_name`=".$occupation_id." AND `DEVICEID`=".$device_id." AND `USERID`=".$USERID;
