...
function loading_labs($USERID,$course_code){
    create_run_device();
    if(!$USERID){
    ...
    $urlhome='/tmp/mnt/iostmp/'.$USERID;
     ...
     if(file_exists($urlhome."/".$course_code)){
         exec("sudo -u root rm -r  ".$urlhome."/".$course_code);
     } 
    ...
    $url_routercourse=glob(URL_ROOT."/www".URL_APPEDND."/storage/routecourses/".$course_code."-*.lib");
    $url_router=$url_routercourse[0];
    $url_user=$url.$USERID."/".$course_code; 
    
    $sql1="select `code`,`title`,`netMap`,`devices_offset` from `course` where `code`=".$course_code;
    $topo_data= api_sql_query_array_assoc($sql1,__FILE__,__LINE__);
    $id     =$topo_data[0]['code'];
    $name   =$topo_data[0]['title'];
    ...
    foreach ($netmap5  as  $val){
        ...
        $net_dev_type= DATABASE::getval("select  `type`  from  `course_connection_vmdisk`  where  `net_dev_id`=".$net_dev_id."  and  `cid`='".$course_code."'");
        if($net_dev_type==2){
            $netmap5_new[]=$val;
        }
    }
    ...
    foreach ($diagram_arr  as  $key=>$value){
        if(in_array($key,$net_dev_ids)){
            ...
            $net_dev_type= DATABASE::getval("select  `type`  from  `course_connection_vmdisk`  where  `net_dev_id`=".$key."  and  `cid`='".$course_code."'");
            if($net_dev_type==2){
    ...
    $sql3 = "select * FROM  `net_devices` where `status`='".$course_code."'";
    ...
    for($j=0;$j<count($devices_data);$j++){
        ...
        $command='sudo -u root /sbin/cloudvmroute.sh  system='.$sql_systemmod.'___LABSNAME='.$course_code.'___USERID='.$USERID.'___PORT='.$PORT.'___DEVICEDTYPE='.$DEVICEDTYPE.'___DEVICEDNAME='.$DEVICEDNAME.'___DEVICEID='.$DEVICEID.'___ROUTETYPE='.$ROUTETYPE.'___IOSFILENAME='.$IOSFILENAME.'___ROUTEMOD='.$ROUTEMOD.'___MEM='.$MEM.'___NVRAM='.$NVRAM.'___idlepc='.$idlepc.'___LINKDATA='.$LINKDATA.'__slot='.$slot;
        $command =  str_replace("____", "___", $command);
        exec("echo $command '&' >> /tmp/www/$USERID'_'$course_code'.sh' ; chmod 777 /tmp/www/$USERID'_'$course_code'.sh' ");
        
    }
	$tophome = $urlhome."/".$course_code;
        if(!file_exists($tophome)){
            exec(" mkdir -p ".$urlhome);
		if(!empty($url_router)){
			$exec_var0="cd ".$urlhome."/;sudo -u root  tar -zxf ".$url_router." ".$course_code;
			exec($exec_var0);
		}
	}
    
        exec("/tmp/www/$USERID'_'$course_code'.sh' > /dev/null & ");
        exec("sleep 2 ;rm -rf  /tmp/www/$USERID'_'$course_code'.sh'");
...
function stoping_labs($USERID,$course_code){
    if(!$USERID){
        $USERID=$_SESSION['_user']['user_id'];
    }
     
    $stop_sql="select `id`,`USERID`,`DEVICEID`, `DEVICEDTYPE` , `PORT`,`uport` from `labs_run_devices` where `course_name`='".$course_code."' and `USERID`=".$USERID;
    ...
    foreach($running_data as $k=>$v){
        ...
        $deletecommand="sudo -u root /sbin/cloudvmroutestop.sh  ".$userId." ".$course_code." ".$userId."_".$course_code."_".$DEVICEDTYPE."_".$deviceId." ".$userId."_".$course_code."_".$PORT;
