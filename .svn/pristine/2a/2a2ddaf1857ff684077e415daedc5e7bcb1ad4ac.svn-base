...
class app
{
	...
	public function basic()
	{
		...
		switch($subaction)
		{
		    case 'scores':
		        ...
		        $page = $this->ev->get('page');
			    $page = $page > 1?$page:1;
			    $basicsStuScores = $this->basic->getStudentsScoresByBasicId($page,10,$basicid);
			    if($basicsStuScores){
			        foreach($basicsStuScores as $key=>$basicsStuScore){
			            $user = $this->user->getUserById($basicsStuScore['ehuserid']);
			            $assingsArr[$key]['userid'] = '第'.($key+1).'名';
			            $assingsArr[$key]['username'] = $user['username'];
			            $assingsArr[$key]['score'] = $basicsStuScore['ehscore'];
			            $assingsArr[$key]['link'] = $basicsStuScore['ehid'];
			        }
			    }
			    $this->tpl->assign('basicid',$basicid);
			    $this->tpl->assign('basicsStuScores',$assingsArr);
			    $this->tpl->display('basic_scores');
		        break;
	...
	public function export(){
	    ...
	    $id = (int)$_GET['id'];
	    $data = $favor->getAllSorcebyid($id);
	    header("Content-type:application/vnd.ms-excel");
	    header("Content-Disposition:filename=test.xls");
	    header("Content-Type: application/vnd.ms-excel; charset=UTF-8");
	    $name = isset($data[0]['ehexam'])?$data[0]['ehexam']:'';
	    ...
	    foreach ($data as $key =>$v){
	        ...
	        <td>'.$v['ehusername'].'</td>
	        <td>'.$v['ehscore'].'</td>
	        <td>http://edu.51elab.com/ems/index.php?exam-app-history-view&ehid='.$v['ehid'].'</td>
	        </tr>';
	    }

	     echo '</table>';
	    
// 	    echo  mb_convert_encoding($name ."\t\n","UTF-8");;
// 	    echo mb_convert_encoding("名次" . "\t","UTF-8");
// 	    echo mb_convert_encoding("用户" . "\t","UTF-8");
// 	    echo mb_convert_encoding("分数" ."\t","UTF-8");
// 	    echo mb_convert_encoding("备注" ."\t\n","UTF-8");
// 	    foreach ($data as $key =>$v){
// 	        echo mb_convert_encoding("第".($key+1).'名' . "\t","UTF-8");
// 	        echo mb_convert_encoding($v['ehusername'] . "\t" . "\t","UTF-8");
// 	        echo 	        mb_convert_encoding($v['ehscore'] ."\t". "\t","UTF-8");
// 	        echo "http://edu.51elab.com/ems/index.php?exam-app-history-view&ehid="
// 	            . $v['ehid'] ."\t\n";
// 	    }
