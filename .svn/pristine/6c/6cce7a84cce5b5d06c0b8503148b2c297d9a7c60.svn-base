...
class basic_exam
{
	...
	public function getStudentsScoresByBasicId($page,$number = 20,$basicId)
	{
	    $data = array(false,'examhistory',array(array("AND","ehbasicid = :ehbasicid",'ehbasicid',$basicId)),'','ehscore DESC');
	...
	{
	    $data = array('basicexam','basic');
	    $sql = $this->pdosql->makeSelect($data);
	    return $this->db->fetch($sql,array('basicexam'));
	}
	...
	public function getQuestionAnswerHistory($keystring)
	{
	    $args = array('answer,ctime','questionhistory',array(array("AND","keystring = :keystring","keystring",$keystring)));
	    $sql = $this->pdosql->makeSelect($args);
		return $this->db->fetch($sql);
	}
	
	public function addQuestionHistory($args,$replace = false)
	{
	    $data = array('questionhistory',$args);
	    $sql = $this->pdosql->makeInsert($data,0,null,$replace);
	    $this->db->exec($sql);
	    //return $this->db->lastInsertId();
	}
