...
//require_once (api_get_path ( SYS_CODE_PATH ) . "lib/xajax/xajax.inc.php");

//课程默认过期时间
$firstExpirationDelay = 31536000; // <- 86400*365    // 60*60*24 = 1 jour = 86400
$id =  intval(getgpc("syllabus_id"));
$_SESSION['syllabus_id'] = $id;
if($id){
...
$form = new FormValidator ( 'update_course', 'get', '', '', null, true );
...
$form->addElement ( 'checkbox', 'tuesday','星期二','', array ('id' => 'tue','width'=>'100px') );
$group = array ();
...
$group [] =  $form->createElement ( 'checkbox', 'class5', null,'晚自习', array ('id' => 'wed5' ) );
...
$tue = unserialize($default['tue']);
$wed = unserialize($default['wed']);
$thu = unserialize($default['thu']);
$fri = unserialize($default['fri']);
$sat = unserialize($default['sat']);
$sun = unserialize($default['sun']);
$default['mon']=$mon;
$default['tue']=$tue;
$default['wed']=$wed;
$default['thu']=$thu;
...
$default['sat']=$sat;
$default['sun']=$sun;
$default['monday']=$mon['monday'];
$default['tuesday']=$tue['tuesday'];
$default['wednesday']=$wed['wednesday'];
$default['thursday']=$thu['thursday'];
$default['friday']=$fri['friday'];
$default['saturday']=$sat['saturday'];
$default['sunday']=$sun['sunday'];
...
if ($form->validate ()) {
    ...
    $sql = Database::sql_update ( $table_syllabus, $syllabus_data ,"id='$id'");
    //echo $sql;
    $result = api_sql_query ( $sql, __FILE__, __LINE__ );

    tb_close ( "../syllabus/syllabus_list.php" );
