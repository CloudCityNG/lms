...
$sql = "SELECT distinct `code` FROM `vslab`.`report` where `marking_status`=1 and `type`=1 ";
...
$form->addElement ( 'select', 'cc', '我的课程', $all_courses );
...
if ($form->validate ()) {
    ...
    $cc = trim ( $export ['cc'] );
    $data = array ();
//    $data [] = array ('id', "report_name", "user", 'code', "submit_date", "score", "comment", "return" );
    $data [] = array ('编号', "实验报告名称", "用户名", '课程', "时间", "得分", "评语", "结果" );
    $filename = 'ExportReport_' . date ( 'YmdHis' ); //导出文件名
    $sql = "select `id`,`report_name`,`user`,`code`,`submit_date`,`score`,`comment`,`return` from `vslab`.`report` where `marking_status`=1 and `type`=1 ";
    if($cc!=''){
        $sql .= " AND `code`='".$cc."'"; 
    }
