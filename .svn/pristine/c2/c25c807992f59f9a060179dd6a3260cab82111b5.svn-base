...
function  user_filter($code) {
    $user = "select user_id from vmtotal where id = $code";
    
    $result =Database::getval($user , __FILE__, __LINE__);
    
    $result =Database::getval("select `username` from `user` where user_id=".$result, __FILE__, __LINE__);

    return $result;

}

function vm_filter($code) {
    $user = "select vmid from vmtotal where id = $code";
...
function monitor_filter($id, $url_params, $row) {
    $result='';
    $user_id=Database::getval("select `user_id` from `vmtotal` where id=".$row[1], __FILE__, __LINE__);
    $system =Database::getval("select `system` from `vmtotal` where id=".$row[1], __FILE__, __LINE__);
    $addres=$row[1];
...
function close_filter($id) {
    $result = confirm_href ( 'stop_small.png', '您确定要关闭该虚拟实验吗？', '', 'exam_vm.php?action=shutvm&id='.$id);
    return $result;
}
...
//            $output = exec("sudo -u root /usr/bin/ssh root@$addres /sbin/cloudvmstop.sh $vmid");
    ...
    api_sql_query ( $sqla, __FILE__, __LINE__ );
     ...
     $sqle="delete from `vm_rel_exam` where `vm_id` =".$ids;
    api_sql_query ( $sqle, __FILE__, __LINE__ );
}
...
function get_vm_data($from, $number_of_items, $column, $direction) {
    $sql = "select `id` ,`vm_id` , `exam_id`  ,`vm_id` ,`id` ,`id`, `vm_id`FROM vm_rel_exam  ";
    $sql_where = get_sqlwhere ();
    if ($sql_where){
        $sql .= " and " . $sql_where;
    }
...
$table->set_header ( 1, '用户名', false, null, array ('style' => 'width:10%;text-align:center' ) );
$table->set_header ( 2, '项目名称', false, null, array ('style' => 'width:25%;text-align:center' ) );
$table->set_header ( 3, 'vmid', false, null, array ('style' => 'width:10%;text-align:center' ) );
//$table->set_header ( 7, 'port', false, null, array ('style' => 'width:5%;text-align:center' ) );
$table->set_header ( 4, '监控', false, null, array ('style' => 'width:10%;text-align:center' ) );
...
$table->set_header ( 5, '远程协助', false, null, array ('style' => 'width:10%;text-align:center' ) );
$table->set_header ( 6, '关闭', false, null, array ('style' => 'text-align:center' ) );

$table->set_column_filter ( 1, 'user_filter' );
$table->set_column_filter ( 2, 'lesson_filter' );
$table->set_column_filter ( 4, 'monitor_filter' );
$table->set_column_filter ( 3, 'vm_filter' );
$table->set_column_filter ( 5, 'con_filter' );
$table->set_column_filter ( 6, 'close_filter' );
...
<section id="main" class="column">
    <h4 class="page-mark">当前位置：<a href="<?=URL_APPEDND;?>/main/admin/index.php">平台首页</a> &gt; <a href="<?=URL_APPEDND;?>/main/exam/exam_list.php">考试管理</a> &gt;虚拟机管理</h4>
