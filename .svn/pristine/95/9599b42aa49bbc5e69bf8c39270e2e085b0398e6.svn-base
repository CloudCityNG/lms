...
class favor_exam
{
	...
	{
		$t = TIME - 24*3600*7;
		$data = array("count(*) AS number,ehuserid,ehusername,max(ehscore) as ehscore",'examhistory',array(array("AND","ehstarttime >= :ehstarttime",'ehstarttime',$t)),"ehuserid","number DESC",10);
		$sql = $this->pdosql->makeSelect($data);
		return $this->db->fetchAll($sql);
	}
	
	public function getAllSorcebyid($id)
	{
	    $data = array("ehid,ehexam,ehusername,max(ehscore) as ehscore",'examhistory',array(array("AND","ehbasicid = :ehbasicid",'ehbasicid',$id)),"ehuserid","ehscore DESC");
	    $sql = $this->pdosql->makeSelect($data);
	    return $this->db->fetchAll($sql);
	}
	...
	public function getDatabyUidBid($uid,$bid) {
	    $data = array("ehid,ehexam,ehusername,max(ehscore) as ehscore",'examhistory',array(array("AND","ehbasicid = :ehbasicid",'ehbasicid',$bid),array("AND","ehuserid = :ehuserid",'ehuserid',$uid)),"ehuserid","ehscore DESC");
	    $sql = $this->pdosql->makeSelect($data);
	    return $this->db->fetchAll($sql);;
	}
}
