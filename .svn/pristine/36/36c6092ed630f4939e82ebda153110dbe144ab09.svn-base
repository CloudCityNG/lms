<?php // $Id: scorm_12lib.php,v 1.11.2.9 2008/12/22 07:04:37 danmarsden Exp $
...
function scorm_get_toc($user_id,$scorm,$liststyle,$currentorg='',$scoid='',$mode='normal',$attempt='',$play=false) {
    global $CFG;
    ...
    if ($scoes = scorm_get_scoes($scorm->id)){
        ...
        foreach ($scoes as $sco) {
            if (!empty($sco->launch_data)) {
                if ($usertrack = scorm_get_tracks($sco->id,$user_id,$attempt)) {
                    if ($usertrack->status == '') {
            ...
            if (!empty($sco->launch)) {
                if ($isvisible) {
                    ...
                    if (empty($sco->prerequisites) || scorm_eval_prerequisites($sco->prerequisites,$usertracks)) {
                        ...
                        $url = WEB_QH_PATH.'scorm/scorm_player.php?a='.$scorm->id.'&amp;'.$modestr.'&amp;scoid='.$sco->id;
    ...
    $url = WEB_QH_PATH.'scorm/scorm_player.php?a='.$scorm->id.'&amp;'.$modestr.'&amp;scoid=';
