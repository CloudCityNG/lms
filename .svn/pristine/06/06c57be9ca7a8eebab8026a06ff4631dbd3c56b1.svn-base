...
if(isset($_GET['exe_id']) && $_GET['action']=='del_result'){
    $objExerciseTmp = new Exercise ();
    $exe_id = intval ( getgpc ( 'exe_id' ));
    $objExerciseTmp->del_exercise_tracking ( $exe_id );
...
$url = api_add_url_querystring ( 'exercise_result.php?choice=export', $query_vars );
...
if(count($data_list)>0){
    echo '<span style="float:right; padding-top:5px;">'.link_button ( 'excel.gif', '导出考试成绩', $url ).'</span>';
}
$form->display ();
echo '</div>';
...
$table_header [] = array (get_lang ( '是否及格' ), false );
...
function get_result_date($exerciseId, $is_export = FALSE) {
	...
	$sql = "SELECT t1.*,t3.username,t3.firstname,t3.lastname,t3.user_id,t3.official_code,CONCAT(t3.org_name,'/',t3.dept_name) AS deptname FROM $TBL_TRACK_EXERCICES AS t1," . $tbl_user_dept . " AS t3 WHERE exe_exo_id=" . Database::escape ( $exerciseId ) . " AND t1.exe_user_id=t3.user_id";
	...
	while ( $data = Database::fetch_array ( $result, 'ASSOC' ) ) {
		...
		$row [] = $data ['exe_result'];
		$row [] = $data ['exe_weighting'] > 0 ? round ( round($data ['exe_result']) * 100 / round($data ['exe_weighting']) ) : 0;
		$row [] = $data ['score'] < 60 ? "不及格":"及格" ;
                                   if ($is_export == false){
            $act= icon_href ( 'scorm_fullscreen.gif', 'ExamViewSub', '../exam/exam_view.php?result_id=' . $data ['exe_id'] . "&exam_id=" . $exerciseId.'&user_id='.$data["user_id"], '_blank' );
       //     $act.= link_button ( 'edit.gif', 'ModifyScore', 'check_results.php?action=modify_score&user_id='.$data["user_id"].'&id=' . $exerciseId, '30%', '40%', 0 );
            $act.= confirm_href ( 'delete.gif', 'ConfirmYourChoice', 'Delete', 'exercise_result.php?action=del_result&exe_id='.$data ['exe_id'].'&exerciseId='. $exerciseId);
            $row [] =$act;
        }
