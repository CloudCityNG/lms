...
if (isset ($action)) {
    switch ($action) {
        case 'delete' :
            ...
            if ( isset($delete_id)){
                $all_users2 =unserialize(Database::getval("select `red_vm` from `task` where `id`=".$task_id,__FILE__,__LINE__));

                array_remove($all_users2, $delete_id);
                //  var_dump($all_users2);

                $results=serialize($all_users2);
                $sql="UPDATE  `vslab`.`task` SET  `red_vm` ='".$results."' WHERE  `task`.`id` =".$task_id;

                api_sql_query ( $sql, __FILE__, __LINE__ );


                $redirect_url = "red_template.php?id=".$task_id;
                api_redirect ( $redirect_url );
            }
            break;
    }
}
//var_dump( $_POST['target_select']);
...
//    if($str!=''){
        ...
        $all_users =unserialize(Database::getval("select `red_vm` from `task` where `id`=".$taskid,__FILE__,__LINE__));

        if($all_users[0]==''){
            $result= $vm_list;
        }else{
            $intersection = array_diff($all_users,$vm_list);
            $result= array_merge($intersection,$vm_list);//echo '1111';
        }
    ...
    for($p=0;$p<count($result);$p++){
        ...
        $sql_vm="UPDATE  `vslab`.`task` SET  `red_vm` =  '".$result_var."' WHERE  `task`.`id` =".$taskid;
        api_sql_query($sql_vm,__FILE__,__LINE__);
//    }
    $redirect_url = "red_template.php?id=".$taskid;
    api_redirect ( $redirect_url );
}
?>
<br>
<form name="theForm" method="post" action="red_template.php" onsubmit="validate()">
    ...
    <table border="0" cellpadding="5" cellspacing="0" align="center"
        ...
        <tr class="containerBody">
            <td class="formLabel">选择设置的靶机模板</td>
...
//             return true; 
            ...
            window.location.href="red_template.php";
        }
    </script></form>
<?php
...
$user_data =unserialize(Database::getval("select `red_vm` from `task` where `id`=".  intval(getgpc('id')),__FILE__,__LINE__));
...
if($count_users_data!=0){
    for($i=0;$i<$count_users_data;$i++){
        ...
        $row [] = $i+1;
        $row [] = $user_data[$i];
        $href = 'red_template.php?action=delete&code=' .intval($i).'&task_id='.$id;
