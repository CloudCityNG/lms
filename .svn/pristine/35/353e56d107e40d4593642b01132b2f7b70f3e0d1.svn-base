...
class pdosql
{
	...
	//生成update sql
	public function makeUpdate($args,$tablepre = NULL,$replace = false)
	{
		...
		$db_orders = $db_orders?' ORDER BY '.$db_orders:'';
		if($replace){
    		$sql = 'REPLACE INTO '.$db_tables.' SET '.$parsql;
		}else{
	...
	public function makeInsert($args,$dim = 0,$tb_pre = NULL,$repalce = false)
	{
		...
		if($repalce){
    		$sql = 'REPLACE INTO '.$db_tables.' ('.implode(',',$db_field).') VALUES ('.implode(',',$db_value).')';
		}
