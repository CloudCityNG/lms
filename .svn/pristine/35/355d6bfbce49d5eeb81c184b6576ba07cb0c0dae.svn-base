...
//                           $form->addElement ( 'hidden', 'exerciseType', $arr[0], array ('id' => "exe" )); 
                           $form->addElement ( 'select', 'exerciseType', get_lang ( 'ExamProperty' ), $tbl_row, array ('style' => "width:15%" ) );  
		...
		//通过分数 PassScore
		$form->addElement ( 'text', 'pass_score', get_lang ( '及格率' ), array ('maxlength' => 3, 'style' => "width:100px;text-align:left", 'class' => 'inputText', 'onfocus' => 'javascript:this.select();' ) );
...
//		$form->addRule ( 'exerciseAttempts', get_lang ( 'Numeric' ), 'numeric' );
		...
		$sql="select user_id, username from user where is_admin=1 or status=10";
                            $result = api_sql_query ( $sql, __FILE__, __LINE__ );
                             $tbl_row = array ();
                            while ($row = Database::fetch_array ( $result, 'ASSOC' )){
...
//		$this->exam_manager = trim ( $ref_exam_manager ['TO_ID_ADMIN'] );
		...
		$query = "SELECT round(AVG(exe_result),2) FROM  exam_track AS t3 WHERE t3.exe_exo_id=" . Database::escape ( $exerciseId )."AND status = 'completed'";
	 ...
	 * @param $user_id
	 */
	static function do_exam_available($exam_id, $user_id, $exerciseStartTime,$exerciseEndTime) {
		global $_user;
		$allwedAccess = FALSE;
		...
		if (empty ( $exam_info )) { //试卷不存在
			...
			if (Database::num_rows ( $rs ) > 0) { //有记录,表明是考试用户
				...
				// 用户可参加考试时间有特殊安排时
				$sql = "SELECT id,available_start_date,available_end_date FROM $tbl_exam_rel_user WHERE NOW() BETWEEN  '".$exerciseStartTime."'  AND  '".$exerciseEndTime."'  AND exam_id=" . Database::escape ( $exam_id ) . " AND user_id=" . Database::escape ( $user_id );
				...
				//已通过的考试不允许再进入考试了
				 $sql = 'select exe_id from exam_track where exe_exo_id='.$exam_id.' and exe_user_id='.$user_id.' and unix_timestamp(exe_date)>0';
                $max_score = Database::getval ( $sql, __FILE__, __LINE__ );
				if ($max_score) return 106;
				return SUCCESS;
			} else {
				return 103;
			}
