...
function edit_filter($id) {
    ...
    $result .=  link_button ( 'edit.gif', 'Edit', 'renwu_edit.php?id='.intval($id), '90%', '70%', FALSE );
    return $result;
}

function red_filter($id) {
    $result = "";
    $result .=  link_button ( 'calendar_month.gif', '红方用户组', 'red_renwu.php?id='.intval($id), '90%', '70%', FALSE );
    return $result;
}
function blue_filter($id) {
    $result = "";
    $result .=  link_button ( 'calendar_personal.gif', '蓝方用户组', 'blue_renwu.php?id='.intval($id), '90%', '70%', FALSE );
    return $result;
}
...
function delete_filter($id) {
    $result = "";
    $result .= confirm_href ( 'delete.gif', 'ConfirmYourChoice', 'Delete', 'renwu.php?action=delete&id=' . intval($id) );
    return $result;
}
    ...
    switch ($action) {
        case 'delete' :
            ...
            if ( isset($delete_id)){
                ...
                $sql = "DELETE FROM `vslab`.`renwu` WHERE `renwu`.`id` = {$delete_id}";
                $result = api_sql_query ( $sql, __FILE__, __LINE__ );
                if($result){
                    $update_sql ="UPDATE  `vmdisk` SET  `type` =  '0', `tasks_id` =  '0' WHERE  `vmdisk`. `task_id`=".$delete_id;
                    api_sql_query($update_sql,__FILE__,__LINE__);
                }
                $redirect_url = "renwu.php";
        ...
        case 'deletes' :
            ...
            if (count ( $labs ) > 0) {
                foreach ( $labs as $index => $id ) {
                    ...
                    $sql = "DELETE FROM `vslab`.`renwu` WHERE `renwu`.`id` =".intval($id);
                    $result = api_sql_query ( $sql, __FILE__, __LINE__ );
...
function get_number_of_labs_topo() {
    $labs_topo = Database::get_main_table (renwu);
...
function get_labs_topo_data($from, $number_of_items, $column, $direction) {
    ...
    $sql = "select `id`,`id`,`name`,`description`,`id`,`id`,`id`,`id` FROM ".$labs_topo." WHERE 1 ";
...
$table->set_header ( $idx ++, '任务名称', false, null, array ('style' => 'width:25%' ));
$table->set_header ( $idx ++, '任务描述', false, null, array ('style' => 'width:25%;text-align:left' ));
$table->set_header ( $idx ++, '红方小组', false, null, array ('style' => 'width:10%' ));
$table->set_header ( $idx ++, '蓝方小组', false, null, array ('style' => 'width:10%' ));
...
//$table->set_column_filter ( 4, 'group_filter' );
...
<section id="main" class="column">
    <h4 class="page-mark">当前位置：<a href="<?=URL_APPEDND;?>/main/admin/index.php">平台首页</a>
        &gt; <a href="<?=URL_APPEDND;?>/main/admin/control/control_list.php">分组对抗管理</a>
        &gt; 分组任务下发 </h4>
    <div class="managerSearch">
        <?php $form->display ();?>
        <span class="searchtxt right">
        <?php echo '&nbsp;&nbsp;' . link_button ( 'add.gif', '添加', 'renwu_add.php', '90%', '70%' );?>
        </span>
    </div>
