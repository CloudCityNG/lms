...
$form = new FormValidator ( 'report_edit','POST','reports_edit.php?id='.$id,'');
//名称
...
if ($form->validate ()) {
    ...
    $sql = "UPDATE `report` SET `report_name`= '".$report_name."',`score`= '".$score."',`comment`= '".$comment."',`return`= '".$return."', `marking_status`=1, `type`=1 WHERE `id`='".$id."'";
    $result = api_sql_query ( $sql, __FILE__, __LINE__ );
    tb_close ( 'reports.php' );
}
