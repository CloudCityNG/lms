...
function get_number_of_users() {
	$user_table = Database::get_main_table (cn_massage);
...
function modify_filter($id, $url_params) {
	...
	$result .= '&nbsp;' . link_button ( 'edit.gif', 'ModifyUserInfo', 'massage_edit.php?id=' . $id, '70%', '80%', FALSE );
	if (api_is_platform_admin () && ! in_array ( $id )) {
		$result .= '&nbsp;' . confirm_href ( 'delete.gif', 'ConfirmYourChoice', 'Delete', 'massage_list.php?action=delete_faq&id=' . $id );
	}
...
if (isset ( $_GET ['action'] )) {
    $sql="delete from cn_massage where id=".$_GET['id'];
    $ret=mysql_query($sql);
    $id=  intval(getgpc('id'));
...
$url_add_user='massage_add.php?keyword_deptid='.(is_not_blank($_GET['keyword_deptid'])?intval(getgpc('keyword_deptid','G')):intval(getgpc('keyword_orgid','G')));
echo link_button ( 'add_user_big.gif', '新增规则', $url_add_user, '70%', '80%' );
...
$table->set_header ( $idx ++, get_lang ( '说明' ), false, null, array ('style' => 'width:30%' ));
$table->set_header ( $idx ++, get_lang ( '规则' ), false, null, array ('style' => 'width:30%' ));
