...
/*
 ==============================================================================
 WebCS - elearning and course management software
		...
		if($readdir != '..' && $readdir != '.')
		{
			if(is_file($dir.'/'.$readdir))
			{
				...
				if ($pos) {  //so we have the imsmanifest in this dir
					//from d:/myworks/ZLMS/ZLMS_cvs/ZLMS/ZLMS/courses/CVSCODE4/scorm/LP2/LP2
					...
					if ($pos==0) {
						...
						$sql="SELECT * FROM ".Database::get_scorm_table(TABLE_SCORM_MAIN)." where (contentTitle='$scormdir' and ZLMSCourse='$courseid')";
						...
						$sql="DELETE FROM ".Database::get_scorm_table(TABLE_SCORM_MAIN)." where (contentTitle='$scormdir' and ZLMSCourse='$courseid')";
						$result=api_sql_query($sql,__FILE__,__LINE__);
					}
				}
