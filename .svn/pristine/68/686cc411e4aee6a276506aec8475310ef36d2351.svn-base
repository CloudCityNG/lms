...
<?php
if ($top_dept) {
	echo 'd.add(' . $top_dept ['id'] . ',' . $top_dept ['pid'] . ',"' . trim ( $top_dept ['dept_name'] ) . '","' . 'user_list.php' . '","","List");' . "\n";
	foreach ( $dept_tree as $dept ) {
		if ($dept ['pid'] == DEPT_TOP_ID) {
			$tree_url = "user_list.php?keyword_org_id=" . $dept ['id'];
			$dept_name = trim($dept ['dept_name']) . ($user_cnt [$dept ['id']] ? ' (' . $user_cnt [$dept ['id']] . ', ' . $org_user_cnt [$dept ['id']] . ') ' : '');
		} else {
			$tree_url = "user_list.php?keyword_deptid=" . $dept ['id'];
			$dept_name = trim($dept ['dept_name']) . ($user_cnt [$dept ['id']] ? ' (' . $user_cnt [$dept ['id']] . ') ' : '');
		}
