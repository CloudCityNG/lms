...
/**search USER**/
if(isRoot()){
    $sql1 = "SELECT `user_id`,`username` FROM `user` where user_id in (select user_id from course_rel_user where 1) ";
}else{
    $user_id=api_get_user_id();
    $sql1 = "SELECT `user_id`,`username` FROM `user` where user_id in (select user_id from course_rel_user where arrange_user_id=$user_id) ";
}
...
function get_vm_data($from, $number_of_items, $column, $direction) {
    $course_code=intval(getgpc('code'));
    if($course_code){
        $_SESSION['lesson_id']=$course_code;
    }
    $lesson_id=$_SESSION['lesson_id'];
    if(isRoot()){
         $sql = "select `id` ,`addres` , `nicnum` , `system` ,`user_id` ,`lesson_id` ,`vmid` ,`port` ,`monitor` ,`mac_id`, `id`, `id` ,`id` FROM vmtotal WHERE `manage`=0 and lesson_id=".$lesson_id;
    }else{
         $sql = "select `id` ,`addres` , `nicnum` , `system` ,`user_id` ,`lesson_id` ,`vmid` ,`port` ,`monitor` ,`mac_id`, `id`, `id` ,`id` FROM vmtotal WHERE `manage`=0 and lesson_id=".$lesson_id." and user_id in (select user_id from course_rel_user where arrange_user_id=$user_id)";
    }
