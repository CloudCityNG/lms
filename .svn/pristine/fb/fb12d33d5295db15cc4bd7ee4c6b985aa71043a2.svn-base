...
//2004-12-20 陈猛增加
//获得两个日期间的天数之差。
//输入: "YYYY-MM-DD","YYYY-MM-DD"
//
function getcomparedays(date1,date2){
   dateyear=date1.substring(0,4)
   datemonth=date1.substring(5,7)-1
   dateday=date1.substring(8,10)
   var ndate1=new Date(dateyear,datemonth,dateday);
   dateyear=date2.substring(0,4)
   datemonth=date2.substring(5,7)-1
   dateday=date2.substring(8,10)
   var ndate2=new Date(dateyear,datemonth,dateday);
   return (ndate2-ndate1)/86400/1000;
}
//
//2004-12-20 陈猛增加
//获得两个日期时间间的秒数之差。
//输入: "YYYY-MM-DD hh:mi:ss","YYYY-MM-DD hh:mi:ss"
//
function getcomparetimes(date1,date2){
   dateyear=date1.substring(0,4)
   datemonth=date1.substring(5,7)-1
   dateday=date1.substring(8,10)
   datehour=date1.substring(11,13)
   datemin=date1.substring(14,16)
   datesec=date1.substring(17)
   var ndate1=new Date(dateyear,datemonth,dateday,datehour,datemin,datesec);
   dateyear=date2.substring(0,4)
   datemonth=date2.substring(5,7)-1
   dateday=date2.substring(8,10)
   datehour=date2.substring(11,13)
   datemin=date2.substring(14,16)
   datesec=date2.substring(17)
   var ndate2=new Date(dateyear,datemonth,dateday,datehour,datemin,datesec);
   return (ndate2-ndate1)/1000;
}
...
function fPopCalendar(popCtrl, dateCtrl,strDate){
  ...
  with (VicPopCal.style) {
	...
	width = 210; //Modifield by daijinnian/zte 2002-11-18 // Added by Danian Zhang/SUI
...
with (document) {
write("<Div id='VicPopCal' style='OVERFLOW:hidden;POSITION:absolute;VISIBILITY:hidden;border:2px ridge;z-index:100;' ondoubleclick='HiddenDiv();' >");
write("<table border='0' bgcolor='#cccccc'>");
write("<TR>");
write("<td valign='middle' align='center' class='title21'><input type='button' name='PrevMonth' value='<' style='height:20;width:20;FONT:bold' onClick='fPrevMonth()'>");
...
write("<TABLE width='100%'><TR><TD align='center' class='title21'>");
...
write("</td><td algin='center' class='title21'>");
