...
if( $ids!=''){
    $sql="select matchRule from $exam_type where id = '".$ids."'";
...
function check_name( $element_value,$exam_type) {
    $sql="select `matchRule` from ".$exam_type;
...
$form->addElement ( 'textarea', 'matchRule', '描述', array ('type'=>'textarea','rows'=>'15','cols'=>'80' ) );
