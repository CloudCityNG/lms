...
<div id="answer_type_<?=COMBO_UNIQUE_ANSWER ?>"
	...
	class="thickbox"
	href="<?=api_get_path(WEB_CODE_PATH) ?>exercice/admin.php?newQuestion=yes&answerType=1&pid=<?= $get_id?>&exerciseId=<?= is_object($objExercise)?$objExercise->id:"" ?>&KeepThis=true&TB_iframe=true&height=390&width=900&modal=true">
			...
			<tr>
				...
				<table class="data_table">
					...
					<?php if($i%2==0) echo 'class="row_odd"'; else echo 'class="row_even"'; ?>>
						...
						{
							?> <a
							href="<?php echo $_SERVER['PHP_SELF']; ?>?moveUp=<?=$id?>&id=<?=$get_id ?>&exerciseId=<?=$exerciseId ?>">
