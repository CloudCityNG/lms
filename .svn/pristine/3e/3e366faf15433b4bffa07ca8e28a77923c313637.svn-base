	...
	try {
		...
		if (urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png'
			...
			imgPreloader.onload = function() {
				...
				$("#TB_window")
						.append(
								"<a href='' id='TB_ImageOff' title='关闭'><img id='TB_Image' src='"
										+ url
										+ "' width='"
										+ imageWidth
										+ "' height='"
										...
										+ TB_PrevHTML
										+ TB_NextHTML
										+ "</div></div><div id='TB_closeWindow'><a href='#' id='TB_closeWindowButton' title='关闭'>关闭</a></div>");
			...
			if (w.toString().indexOf('%') > -1) {
				TB_WIDTH = parseInt( $(window).width()  * parseFloat(w) / 100, 10);
			} else{
				TB_WIDTH = (params['width'] * 1) + 30 || 630;
			}
			
			if (h.toString().indexOf('%') > -1) {
				TB_HEIGHT = parseInt( $(window).height()  * parseFloat(h) / 100, 10);
			} else{
				TB_HEIGHT = (params['height'] * 1) + 40 || 440;
			}
			//alert(w+" "+TB_WIDTH+", "+h+" "+TB_HEIGHT);
					...
					$("#TB_window")
							.append(
									"<div id='TB_title'><div id='TB_ajaxWindowTitle'>"
											+ caption
											+ "</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton' title='关闭'>关闭</a></div></div><iframe frameborder='0' hspace='0' src='"
											+ urlNoQuery[0]
											...
											+ (ajaxContentH + 17)
											+ "px;' > </iframe>");
				} else {// iframe modal liyu:20091209
					$("#TB_overlay").unbind();
					$("#TB_window")
							.append(
									"<div id='TB_title'><div id='TB_ajaxWindowTitle'>"
											+ caption
											+ "</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton' title='关闭'>关闭</a></div></div>"
					...
					if (params['modal'] != "true") {// ajax no modal
						$("#TB_window")
								.append(
										"<div id='TB_title'><div id='TB_ajaxWindowTitle'>"
												+ caption
												+ "</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton' title='关闭'>关闭</a></div></div><div id='TB_ajaxContent' style='width:"
