...
include_once ('../lib/inc/export.lib.inc.php');
...
function content_filter($id){
    ...
    $result.="<a href='flag_report.php?action=view&id=".$id."'><span style='text-align:left'>".$comment."</span></a>";
    return $result;
}
...
if($g_action=='view' && $g_id!==''){
    ...
    $file = URL_ROOT.'/www/'.URL_APPEDND.'/storage/report/flag/'.$info['user'].'/'.$info['screenshot_file'];
    if (file_exists($file)) {
...
function action_filter($id){
    ...
    $marking_status =Database::getval("select `marking_status` from `vslab`.`reporting_info` where `id`=".$id,__FILE__,__LINE__);
    if($marking_status!=1){
        $result .=  link_button ( 'plugin.gif', '夺旗报告批改', 'reporting_edit.php?id=' . $id, '100%', '70%', FALSE );
    }
    $result .=  confirm_href ( 'delete.gif', '你确定执行此操作？', 'Delete', 'flag_report.php?action=delete&delete_id=' . $id );
    return $result;
}
...
function get_number_of_report() {
    ...
    $sql = "SELECT COUNT(id) AS total_number_of_items FROM " . $report." where `status`=1 and type=1 ";
...
function get_report_data($from, $number_of_items, $column, $direction) {
    $report = Database::get_main_table ( reporting_info );
    $sql = "select `id`,`report_name`,`user`,`id`,`key`,`score`,`comment`,`id` ,`id` FROM  ".$report." where `status`=1 and type=1 ";
...
$table->set_header ( $idx ++, '夺旗报告名称', false  ,null, array ('style' => ' text-align:center;width:15%' ));
...
<section id="main" class="column">
    <h4 class="page-mark">当前位置：<a href="<?=URL_APPEDND;?>/main/admin/index.php">平台首页</a> &gt;
        <a href="<?=URL_APPEDND;?>/main/reporting/report.php">实验报告</a>
        &gt; 夺旗报告管理</h4>
    ...
    <div class="managerSearch">
        ...
        <?php
            ...
            if($report_count > 0){
                echo '&nbsp;&nbsp;' . link_button ( 'return.gif', '导出报告', 'fc_report_export.php?type=1', '25%', '25%' );
            }
            ?>
        </span>
    </div>
