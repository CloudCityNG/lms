...
$_SESSION['red_task'] =  htmlspecialchars($_GET ['id']);
$id = intval($_SESSION['red_task']);
...
//                $sql = "UPDATE  `vslab`.`user` SET  `type` =  '0',group_id='0' WHERE  `user`.`user_id` ={$delete_id}";
                $sql = "UPDATE  `vslab`.`user` SET  group_id='0' WHERE  `user`.`user_id` ={$delete_id}";
...
if(isset($_POST['formSent']) && getgpc("formSent","P")==1){
    ...
    $red_tem=explode(",",$str);
//    echo '<pre>';var_dump($red_tem);echo "</pre>";
    foreach($red_tem as $k){
        ...
        $sql_vm="UPDATE  `vslab`.`user` SET  `group_id` = '".$userid."' WHERE  username='".$username."';";
...
?>
<br>
<form name="theForm" method="post" action="blue_user_control.php" onsubmit="validate()">
                    ...
                    <tr>
                        <td>
                            ...
                            <select name="source_select[]" size="10" id="source_select[]" style="width: 250px;" ondblclick="moveItem_l2r(G('source_select[]'),G('target_select[]'),false)" multiple="true">
                                <?php
                                $sql="select user_id,username,firstname,dept_id,status from user where status=5 and group_id=0";
                                $re=api_sql_query ( $sql, __FILE__, __LINE__ );

                                while($arr=mysql_fetch_row($re)){
                                     $dept=Database::getval("select dept_name from sys_dept where id=".intval($arr[3]),__FILE__,__LINE__);
                                    echo "<option>".$arr[1]."(".$arr[2].")--".$dept."</option>";
                                }
                                ?>
                            </select>
                        </td>
    ...
    <script type="text/javascript">
        ...
        function validate(){
            ...
            window.location.href="blue_user_control.php";
        }
    </script></form>
<?php

function get_user_data($condition = '') {
    $sql = "SELECT user_id,status,username,firstname,dept_id,user_id FROM user where group_id=".  intval(getgpc('id'));
...
$table_header [] = array (get_lang ( '登录名称' ), true );
$table_header [] = array (get_lang ( '真实名称' ), true );
...
//    }
    ...
    $dept=Database::getval("select dept_name from sys_dept where id=".intval($data['dept_id']),__FILE__,__LINE__);

    $row [] = $data ['user_id'];
    $row [] = $data ['status'];
    $row [] = $data ['username']."&nbsp;(".$dept.")";
    $row [] = $data ['firstname'];
//    $row [] = $data ['type'];
