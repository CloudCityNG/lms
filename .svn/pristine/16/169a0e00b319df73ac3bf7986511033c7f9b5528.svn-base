...
function content_filter($id){    
    $result = "";
    $result=link_button('synthese_view.gif', '浏览实验报告', 'report_view.php?id='.$id, '70%', '80%', FALSE);
    return $result;
}
...
function action_filter($id){
    ...
    $marking_status =Database::getval("select `marking_status` from `vslab`.`report` where `id`=".intval($id),__FILE__,__LINE__);
    if($marking_status==0){
        $result .=  link_button ( 'plugin.gif', '批改实验报告', 'report_edit.php?id=' . intval($id), '100%', '70%', FALSE );
    }
    $result .=  confirm_href ( 'delete.gif', '你确定执行此操作？', 'Delete', 'report.php?action=delete&delete_id=' . $id );
    return $result;
}
...
function get_report_data($from, $number_of_items, $column, $direction) {
    $report = Database::get_main_table ( report );
    $sql = "select `id`,`report_name`,`user`,`id`,`score`,`comment`,`id` ,`id` FROM  ".$report." where `status`=1 ";
...
$table->set_header ( $idx ++, '实验报告名称', false  ,null, array ('style' => ' text-align:center;width:30%' ));
...
$table->set_header ( $idx ++, '浏览', false, null, array ('style' => ' text-align:center;width:8%' ) );
...
$table->set_header ( $idx ++, '教师评语', false, null, array ('style' => 'width:35%' ) );
...
$table->set_column_filter ( 5, 'comment_filter' );
$table->set_column_filter ( 6, 'return_filter' );
$table->set_column_filter ( 7, 'action_filter' );
...
<section id="main" class="column">
    <h4 class="page-mark">当前位置：<a href="<?=URL_APPEDND;?>/main/admin/index.php">平台首页</a> &gt;
        <a href="<?=URL_APPEDND;?>/main/admin/course/report.php">实验报告管理</a> &gt; 实验报告管理</h4>

    <div class="managerSearch">
        <?php $form->display ();?>
        <span class="searchtxt right">
        <?php
            $report_count=Database::getval('SELECT COUNT(id) FROM `report` where `marking_status`=1',__FILE__,__LINE__);
            if($report_count > 0){
                echo '&nbsp;&nbsp;' . link_button ( 'return.gif', '导出实验报告', 'report_export.php', '25%', '25%' );
            }
            ?>
        </span>
    </div>
