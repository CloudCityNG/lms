...
//
//    $sql = "SELECT `id`,`report_name`,`user`, `screenshot_file`,`type` FROM `reporting_info` WHERE id=" . $_GET['ids'];
...
$sql1 = "SELECT COUNT(id) from `reporting_info`  where `type` = 1  and`user`='" .$username."' ";
$sql_where = get_sqlwhere ();
if ($sql_where) $sql1 .= " AND " . $sql_where;
$total_rows = Database::getval ( $sql1, __FILE__, __LINE__ );

$sql = "select `id`,`report_name`,`user`,`submit_date`,`screenshot_file`,`description`,`key`,`status` FROM `vslab`.`reporting_info`   where `type` = 1 and `user`='" .$username."' ";
...
$url = WEB_QH_PATH . "flag_report.php?" . $param;
...
$interbreadcrumb [] = array (  "name" => "我的夺旗报告" );
//$nameTools="我的课程";
        ...
        case 'delete' :
            ...
            if ( isset($delete_id)){
                $file=Database::getval('select `screenshot_file` from `reporting_info` where `id`='.$delete_id,__FILE__,__LINE__);
                    ...
                    if($result){
                        $get_files=URL_ROOT.'/www/'.URL_APPEDND.'/storage/report/flag/'.$_SESSION['_user']['username'].'/'.$file;
                        unlink($get_files);
                    }

                    $redirect_url = "flag_report.php";
        ...
        <header>
            ...
            <div class="submit_link">
                <form action="assignment_list.php" method="get">
                    ...
                    <?php
                    echo '<span style="color:black"><b>'.link_button ( 'create.gif', '添加夺旗报告', 'reporting_add.php?type=1', '90%', '80%' )."</b></span>";
                    ?>
                </form>
            ...
            <table cellspacing="0" cellpadding="0" class="p-table">
                ...
                foreach ( $data_list as $item ) {
                    ...
                    <tr>
                        ...
                        <td width="10%"><?= $item ['report_name'] ?></td>
                        <td width="10%"><?= $item ['submit_date'] ?></td>
                        <td width="15%"><?= $item ['screenshot_file'] ?></td>
                        ...
                        <td width="8%">
                           ...
                           $status= Database::getval("select `status` from `reporting_info` where `id`='".$item ['id']."'",__FILE__,__LINE__);
                            if($status==1){
                                echo link_button ( 'statistics.gif', '查看夺旗报告', 'reporting_info.php?action=info&id='.$item ['id'], '70%', '60%', FALSE );
                            }else{
                                echo '<a href="flag_report.php?action=submit_report&id='.$item_id.'" title="提交夺旗报告结果"><img src="../../themes/img/folder_up.gif" alt="提交夺旗报告结果" title="提交夺旗报告结果" style="vertical-align: middle;"></a>';
                                echo link_button ( 'exercise22.png', '编辑夺旗报告结果', 'reporting_edit.php?action=edit&id='.$item ['id'], '90%', '960', FALSE );
                                echo confirm_href ( 'delete.gif', '你确定执行此操作？', 'Delete', 'flag_report.php?action=delete&delete_id=' . $item ['id'] );
                            }
                            ?></td>
                    </tr>
                    <?php
