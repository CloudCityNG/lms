...
function loadcalendar() {
	...
	for(var k = 1930; k <= 2019; k++) {
		...
		s += '<a href="###" onclick="refreshcalendar(' + k + ', mm);document.getElementById(\'calendar_year\').style.display=\'none\'"><span' + (today.getFullYear() == k ? ' class="bold"' : '') + ' id="calendar_year_' + k + '">' + k + '</span></a><br />';
	}
	s += '</div></div>';
	s += '<div id="calendar_month" onclick="_cancelBubble(event)" style="display: none; padding: 3px; z-index: 140" class="calendarmenu">';
	for(var k = 1; k <= 12; k++) {
		s += '<a href="###" onclick="refreshcalendar(yy, ' + (k - 1) + ');document.getElementById(\'calendar_month\').style.display=\'none\'; "><span' + (today.getMonth()+1 == k ? ' class="bold"' : '') + ' id="calendar_month_' + k + '">' + k + ( k < 10 ? '&nbsp;' : '') + ' 月</span></a><br />';
	}
...
function showcalendar(addtime1, startdate1, enddate1) {
	...
	if(lastcheckedyear != false) {
		document.getElementById('calendar_year_' + lastcheckedyear).className = '';
		document.getElementById('calendar_year_' + today.getFullYear()).className = 'bold';
	}
	if(lastcheckedmonth != false) {
		document.getElementById('calendar_month_' + lastcheckedmonth).className = '';
		document.getElementById('calendar_month_' + (today.getMonth() + 1)).className = 'bold';
	}
	document.getElementById('calendar_year_' + currday.getFullYear()).className = 'error bold';
	document.getElementById('calendar_month_' + (currday.getMonth() + 1)).className = 'error bold';
