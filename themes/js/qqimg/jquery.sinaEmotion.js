		...
		success : function(response) {
			...
			for ( var i in data) {
				...
				emotions[data[i].category].push( {
					name : data[i].phrase,
					icon : '../../storage/qqimg_files/'+data[i].icon
				});
			}
		}
	});
...
(function($){
	$.fn.SinaEmotion = function(target){
		...
		$.fn.insertText = function(text){
			this.each(function() {
				if(this.tagName !== 'INPUT' && this.tagName !== 'TEXTAREA' && this.tagName !== 'DIV') {return;}
					...
					str= this.value.substring(0, start)+ text+ this.value.substring(end, this.value.length);
					$(this).append(str); 
		...
		function showEmotions(){
			...
			$('#emotions .container a').click(function(){
				target.insertText($(this).children('img'));
				$('#emotions').remove();
			});
