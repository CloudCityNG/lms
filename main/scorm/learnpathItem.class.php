...
class learnpathItem{
	...
	var $debug = SCORM_DEBUG; //logging param
	var $course_code="";
	...
	function learnpathItem($db_id, $user_id) {
		//get items table
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem constructor: '.$db_id.','.$user_id,__FILE__,__LINE__);}
		...
		if(Database::num_rows($res)<1)
		{
			...
			api_scorm_log('New LP - '.$this->error);
			return false;
		}
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::add_child()',__FILE__,__LINE__);}
		if(!empty($item))
		{
	...
	function close()
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::close()',__FILE__,__LINE__);}
	...
	function delete()
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpath_item::delete() for item '.$this->db_id,__FILE__,__LINE__);}
		...
		$row = Database::fetch_array($res_sel,'ASSOC');
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::drop_child()',__FILE__,__LINE__);}
		if(!empty($item))
		{
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_attempt_id() on item '.$this->db_id,__FILE__,__LINE__);}
		$res = 1;
		if(!empty($this->attempt_id))
		{
			$res = $this->attempt_id;
		}
		if($this->debug>0){api_scorm_log('New LP - End of learnpathItem::get_attempt_id() on item '.$this->db_id.' - Returning '.$res,__FILE__,__LINE__);}
		return $res;
	}
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_children()',__FILE__,__LINE__);}
		$list = array();
	 ...
	 * autorization. Credit tells the sco(content) if ZLMS will record the data it is sent (credit) or not (no-credit)
	 ...
	 */
	function get_credit(){
		if(!empty($this->debug) && $this->debug>1){api_scorm_log('New LP - In learnpathItem::get_credit()',__FILE__,__LINE__);}
		$credit = 'credit';
		...
		if($this->get_prevent_reinit() != 0){ //if prevent_reinit == 1 (or more)
			...
			if(!empty($this->debug) && $this->debug>2){
				api_scorm_log('New LP - In learnpathItem::get_credit() - get_prevent_reinit!=0 and status is '.$status,__FILE__,__LINE__);
			}
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_current_start_time()',__FILE__,__LINE__);}
	...
	function get_description(){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_description()',__FILE__,__LINE__);}
	...
	function get_file_path($path_to_scorm_dir=''){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_file_path()',__FILE__,__LINE__);}
		$path = $this->get_path();
		$type = $this->get_type();
		if(empty($path)){
			if($type == 'webcs_chapter' OR $type=='chapter' OR $type == 'dir') {
				return '';
			}else{
				return '-1';
			}
		}elseif($path == strval(intval($path))){
			//the path is numeric, so it is a reference to a ZLMS object
			switch($type)
			{
				case 'webcs_chapter':
				case 'dir':
	 ...
	 */
	function get_id(){
		if($this->debug>1){api_scorm_log('New LP - In learnpathItem::get_id()',__FILE__,__LINE__);}
		if(!empty($this->db_id))
		{
			return $this->db_id;
		}
	...
	function load_interactions() {
		...
		if (Database::num_rows($res)>0) {
			...
			$iva_sql = "SELECT * FROM $iva_table  WHERE lp_iv_id = $lp_iv_id ";
	...
	{
		if($this->debug>1){api_scorm_log('New LP - In learnpathItem::get_interactions_count()',__FILE__,__LINE__);}
		$return = 0;
		if ($checkdb) {
			$tbl = Database::get_course_table(TABLE_LP_ITEM_VIEW);
			$sql = "SELECT id FROM $tbl " . " WHERE lp_item_id = ".$this->db_id." " .
			...
			if (Database::num_rows($res)>0) {
				...
				$lp_iv_id = $row[0];
				$iva_table = Database::get_course_table(TABLE_LP_IV_INTERACTION);
				$iva_sql = "SELECT count(id) as mycount FROM $iva_table "."WHERE lp_iv_id = ".Database::escape($lp_iv_id);
				$res_sql = api_sql_query($iva_sql,__FILE__,__LINE__);
	...
	{
		if($this->debug>1){api_scorm_log('New LP - In learnpathItem::get_objectives_count()',__FILE__,__LINE__);}
		$res = 0;
	 ...
	 * @return	string	Launch data as found in imsmanifest and stored in ZLMS (read only). Defaults to ''.
	 */
	function get_launch_data(){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_launch_data()',__FILE__,__LINE__);}
	...
	function get_lesson_location(){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_lesson_location()',__FILE__,__LINE__);}
		if(!empty($this->lesson_location)){return $this->lesson_location;}else{return '';}
	}


	/**
	 * Gets the lesson_mode (scorm feature, but might be used by aicc as well as ZLMS paths)
	 *
	 * The "browse" mode is not supported yet (because there is no such way of seeing a sco in ZLMS)
	 ...
	 */
	function get_level(){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_level()',__FILE__,__LINE__);}
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_mastery_score()',__FILE__,__LINE__);}
	 ...
	 */
	function get_max(){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_max()',__FILE__,__LINE__);}
		if($this->type == 'sco')
		{
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_max_time_allowed()',__FILE__,__LINE__);}
	 ...
	 */
	function get_min(){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_min()',__FILE__,__LINE__);}
	 ...
	 */
	function get_parent(){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_parent()',__FILE__,__LINE__);}
	 ...
	 */
	function get_path(){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_path()',__FILE__,__LINE__);}
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_prereq_string()',__FILE__,__LINE__);}
	...
	function get_prevent_reinit(){
		if($this->debug>2){api_scorm_log('New LP - In learnpathItem::get_prevent_reinit()',__FILE__,__LINE__);}
		if(!isset($this->prevent_reinit)){
			if(!empty($this->lp_id)){
				...
				if(Database::num_rows($res)<1){
					...
					if($this->debug>2){api_scorm_log('New LP - End of learnpathItem::get_prevent_reinit() - Returning false',__FILE__,__LINE__);}
					return false;
				}else{
					$row = Database::fetch_array($res,'ASSOC');
		...
		if($this->debug>2){api_scorm_log('New LP - End of learnpathItem::get_prevent_reinit() - Returned '.$this->prevent_reinit,__FILE__,__LINE__);}
	 ...
	 * @param	string	type (one of the ZLMS tools) - optional (otherwise takes the current item's type)
	 ...
	 */
	function get_score(){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_score()',__FILE__,__LINE__);}
		$res = 0;
		if(!empty($this->current_score))
		{
			$res = $this->current_score;
		}
		if($this->debug>1){api_scorm_log('New LP - Out of learnpathItem::get_score() - returning '.$res,__FILE__,__LINE__);}
		return $res;
	}
	...
	function get_status($check_db=true,$update_local=false) {
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_status() on item '.$this->db_id,__FILE__,__LINE__);}
		if($check_db) {
			if($this->debug>2){api_scorm_log('New LP - In learnpathItem::get_status(): checking db',__FILE__,__LINE__);}
			$table = Database::get_course_table(TABLE_LP_ITEM_VIEW);
			$sql = "SELECT * FROM $table WHERE id = '".$this->db_item_view_id."' AND view_count = '".$this->get_attempt_id()."'";
			if($this->debug>2){api_scorm_log('New LP - In learnpathItem::get_status() - Checking DB: '.$sql,__FILE__,__LINE__);}
			...
			if (Database::num_rows($res)==1) {
				...
				if($this->debug>2){api_scorm_log('New LP - In learnpathItem::get_status() - Returning db value '.$row['status'],__FILE__,__LINE__);}
				return $row['status'];
			}
		} else {
			if($this->debug>2){api_scorm_log('New LP - In learnpathItem::get_status() - in get_status: using attrib',__FILE__,__LINE__);}
			if(!empty($this->status)) {
				if($this->debug>2){api_scorm_log('New LP - In learnpathItem::get_status() - Returning attrib: '.$this->status,__FILE__,__LINE__);}
				return $this->status;
			}
		}
		if($this->debug>2){api_scorm_log('New LP - In learnpathItem::get_status() - Returning default '.$this->possible_status[0],__FILE__,__LINE__);}
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_suspend_data()',__FILE__,__LINE__);}
		//TODO : improve cleaning of breaklines ... it works but is it really a beautiful way to do it ?
		$suspend_data="";
		if(!empty($this->current_data)){
			$suspend_data=str_replace(array("\r","\n"),array('\r','\n'),$this->current_data);
		}else{
	 ...
	 */
	function get_title(){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_title()',__FILE__,__LINE__);}
	...
	function get_total_time(){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_total_time()',__FILE__,__LINE__);}
		if($this->current_start_time == 0){ //shouldn't be necessary thanks to the open() method
			$this->current_start_time = time();
		}
		//$this->current_stop_time=time();  //liyu: 20091205
		$time = $this->current_stop_time - $this->current_start_time;
		if($time < 0){
			return 0;
		}else{
			if($this->debug>2){api_scorm_log('New LP - In learnpathItem::get_total_time() - Current stop time = '.$this->current_stop_time.', current start time = '.$this->current_start_time.' Returning '.$time,__FILE__,__LINE__);}
			return $time;
		}
	}
	...
	{
		$res = 'asset';
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_type() on item '.$this->db_id,__FILE__,__LINE__);}
		if(!empty($this->type))
		{
			//error_log('In item::get_type() - returning '.$this->type,0);
			$res = $this->type;
		}
		if($this->debug>2){api_scorm_log('New LP - In learnpathItem::get_type() - Returning '.$res.' for item '.$this->db_id,__FILE__,__LINE__);}
		return $res;
	}
	...
	function get_view_count(){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::get_view_count()',__FILE__,__LINE__);}
		if(!empty($this->attempt_id)){
	...
	function is_done(){
		if($this->status_is(array('completed','passed','succeeded'))){
			if($this->debug>2){api_scorm_log('New LP - In learnpath::is_done() - Item '.$this->get_id().' is complete',__FILE__,__LINE__);}
			return true;
		}else{
			if($this->debug>2){api_scorm_log('New LP - In learnpath::is_done() - Item '.$this->get_id().' is not complete',__FILE__,__LINE__);}
			return false;
		}
	}
	...
	{
		if($this->debug>2){api_scorm_log('New LP - In learnpathItem::is_restart_allowed()',__FILE__,__LINE__);}
		$restart = 1;
		...
		if($this->debug>2){api_scorm_log('New LP - End of learnpathItem::is_restart_allowed() - Returning '.$restart,__FILE__,__LINE__);}
		return $restart;
	}
	...
	function open($allow_new_attempt=false){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::open()',__FILE__,__LINE__);}
	...
	function output()
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::output()',__FILE__,__LINE__);}
	 ...
	 * @param	integer	The user ID. In some cases like ZLMS quizzes, it's necessary to have the user ID to query other tables (like the results of quizzes)
	...
	function parse_prereq($prereqs_string, $items, $refs_list,$user_id){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::parse_prereq() for learnpath '.$this->lp_id.' with string '.$prereqs_string,__FILE__,__LINE__);}
		...
		if(strpos($prereqs_string,"|")===false){
			if($this->debug>1){api_scorm_log('New LP - Didnt find any OR, looking for AND',__FILE__,__LINE__);}
			if(strpos($prereqs_string,"&")!==false){
				...
				if(count($list)>1){
					...
					foreach($list as $condition){
						...
						if($andstatus==false){
							if($this->debug>1){api_scorm_log('New LP - One condition in AND was false, short-circuit',__FILE__,__LINE__);}
							break;
						}
					}
				...
				if($this->debug>1){api_scorm_log('New LP - Didnt find any AND, looking for =',__FILE__,__LINE__);}
				if(strpos($prereqs_string,"=")!==false){
					if($this->debug>1){api_scorm_log('New LP - Found =, looking into it',__FILE__,__LINE__);}
					...
					if($this->debug>1){api_scorm_log('New LP - Didnt find any =, looking for <>',__FILE__,__LINE__);}
					if(strpos($prereqs_string,"<>")!==false){
						if($this->debug>1){error_log('New LP - Found <>, looking into it',__FILE__,__LINE__);}
						//we assume '<>' signs only appear when there's nothing else around
						$params = split('<>',$prereqs_string);
						if(count($params) == 2){
							//right number of operands
						...
						if(strpos($prereqs_string,"~")!==false){
							...
							if($myres){
								...
								//strange...
								if($this->debug>1){api_scorm_log('New LP - Found ~ but strange string: '.$prereqs_string,__FILE__,__LINE__);}
							}
						}else{

							//Finally, look for sets/groups

							if($this->debug>1){api_scorm_log('New LP - Didnt find any ~, looking for groups',__FILE__,__LINE__);}
							//only groups here
							$groups = array();
							...
							if($groups_there){
								...
								{
									if($this->debug>1){api_scorm_log('New LP - Dealing with group '.$gr,__FILE__,__LINE__);}
									$multi = array();
									$mycond = false;
									if(preg_match('/(\d+)\*\{([^\}]+)\}/',$gr,$multi)){
										if($this->debug>1){api_scorm_log('New LP - Found multiplier '.$multi[0],__FILE__,__LINE__);}
										$count = $multi[1];
										$list = split(',',$multi[2]);
										$mytrue = 0;
										foreach($list as $cond){
											if(isset($items[$refs_list[$cond]])){
												...
												if (($status == $this->possible_status[2]) OR ($status == $this->possible_status[3])){
													$mytrue ++;
													if($this->debug>1){api_scorm_log('New LP - Found true item, counting.. ('.($mytrue).')',__FILE__,__LINE__);}
												}
											}else{
												if($this->debug>1){api_scorm_log('New LP - item '.$cond.' does not exist in items list',__FILE__,__LINE__);}
											}
										}
										if($mytrue >= $count){
											if($this->debug>1){api_scorm_log('New LP - Got enough true results, return true',__FILE__,__LINE__);}
											$mycond = true;
										}else{
											if($this->debug>1){api_scorm_log('New LP - Not enough true results',__FILE__,__LINE__);}
										}
									}
									else{
										if($this->debug>1){api_scorm_log('New LP - No multiplier',__FILE__,__LINE__);}
										$list = split(',',$gr);
										$mycond = true;
										foreach($list as $cond){
											if(isset($items[$refs_list[$cond]])){
												...
												if (($status == $this->possible_status[2]) OR ($status == $this->possible_status[3])){
													$mycond = true;
													if($this->debug>1){api_scorm_log('New LP - Found true item',__FILE__,__LINE__);}
												}else{
													if($this->debug>1){api_scorm_log('New LP - Found false item, the set is not true, return false',__FILE__,__LINE__);}
													$mycond = false;
													break;
												}
											}else{
								...
								if($this->debug>1){api_scorm_log('New LP - Didnt find any group, returning value for '.$prereqs_string,__FILE__,__LINE__);}
								if(isset($items[$refs_list[$prereqs_string]])){
									...
									else
									{
										...
										if(!$returnstatus){
											if($this->debug>1){api_scorm_log('New LP - Prerequisite '.$prereqs_string.' not complete',__FILE__,__LINE__);}
										}else{
											if($this->debug>1){api_scorm_log('New LP - Prerequisite '.$prereqs_string.' complete',__FILE__,__LINE__);}
										}
										return $returnstatus;
									}
								}else{
									if($this->debug>1){api_scorm_log('New LP - Could not find '.$prereqs_string.' in '.print_r($refs_list,true),__FILE__,__LINE__);}
								}
							}
						}
					}
			...
			$list = split("\|",$prereqs_string);
			if(count($list)>1){
				if($this->debug>1){api_scorm_log('New LP - Found OR, looking into it',__FILE__,__LINE__);}
				$orstatus = false;
				foreach($list as $condition){
					if($this->debug>1){api_scorm_log('New LP - Found OR, adding it ('.$condition.')',__FILE__,__LINE__);}
					$orstatus = $orstatus || $this->parse_prereq($condition,$items,$refs_list,$user_id);
					if($orstatus == true){
						//shortcircuit OR
						if($this->debug>1){api_scorm_log('New LP - One condition in OR was true, short-circuit',__FILE__,__LINE__);}
						break;
					}
				}
				...
				if($this->debug>1){api_scorm_log('New LP - OR was found but only one elem present !?',__FILE__,__LINE__);}
				if(isset($items[$refs_list[$list[0]]])){
					$status = $items[$refs_list[$list[0]]]->get_status(true);
		...
		if($this->debug>1){api_scorm_log('New LP - End of parse_prereq. Error code is now '.$this->prereq_alert,__FILE__,__LINE__);}
		return false;
	}
	...
	function parse_HTML_attributes($attrString,$wanted=array())
	{
		...
		try {
			...
			api_scorm_log('Caught exception: '. $e->getMessage(),__FILE__,__LINE__) ;
		}
		if ($res) {
			...
			api_scorm_log('preg_match did not find anything',__FILE__,__LINE__);
		}
		return $attributes;
	}
	...
	function restart()
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::restart()',__FILE__,__LINE__);}
		$this->save();
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::save()',__FILE__,__LINE__);}
		...
		if($this->type == 'sco' || $this->type== 'au'){
			...
			$s != $this->possible_status[0] AND $s != $this->possible_status[1]){
				if($this->debug>1){api_scorm_log('New LP - In learnpathItem::save() - save reinit blocked by setting',__FILE__,__LINE__);}
				...
				if($this->debug>1){api_scorm_log('New LP - In learnpathItem::save() - SCORM save request received',__FILE__,__LINE__);}
				//get all new settings from the URL
				if($from_outside==true){
					if($this->debug>1){api_scorm_log('New LP - In learnpathItem::save() - Getting item data from outside',__FILE__,__LINE__);}
					foreach($_GET as $param => $value)
					{
						...
						switch($param){
							case 'score':
								...
								if($this->debug>2){api_scorm_log('New LP - In learnpathItem::save() - setting score to '.$value,__FILE__,__LINE__);}
								break;
							case 'max':
								$this->set_max_score($value);
								if($this->debug>2){api_scorm_log('New LP - In learnpathItem::save() - setting view_max_score to '.$value,__FILE__,__LINE__);}
								break;
							case 'min':
								$this->min_score = $value;
								if($this->debug>2){api_scorm_log('New LP - In learnpathItem::save() - setting min_score to '.$value,__FILE__,__LINE__);}
								break;
							case 'lesson_status':
								if(!empty($value)){
									$this->set_status($value);
									if($this->debug>2){api_scorm_log('New LP - In learnpathItem::save() - setting status to '.$value,__FILE__,__LINE__);}
								}
								break;
							case 'time':
								$this->set_time($value);
								if($this->debug>2){api_scorm_log('New LP - In learnpathItem::save() - setting time to '.$value,__FILE__,__LINE__);}
								break;
							case 'suspend_data':
								$this->current_data = $value;
								if($this->debug>2){api_scorm_log('New LP - In learnpathItem::save() - setting suspend_data to '.$value,__FILE__,__LINE__);}
								break;
							case 'lesson_location':
								$this->set_lesson_location($value);
								if($this->debug>2){api_scorm_log('New LP - In learnpathItem::save() - setting lesson_location to '.$value,__FILE__,__LINE__);}
								break;
							case 'core_exit':
								$this->set_core_exit($value);
								if($this->debug>2){api_scorm_log('New LP - In learnpathItem::save() - setting core_exit to '.$value,__FILE__,__LINE__);}
								break;
							case 'interactions':
					...
					if($this->debug>1){api_scorm_log('New LP - In learnpathItem::save() - Using inside item status',__FILE__,__LINE__);}
		...
		//$time = $this->time
		if($this->debug>1){api_scorm_log('New LP - End of learnpathItem::save() - Calling write_to_db()',__FILE__,__LINE__);}
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_attempt_id()',__FILE__,__LINE__);}
	...
	function set_description($string=''){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_description()',__FILE__,__LINE__);}
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_lesson_location()',__FILE__,__LINE__);}
		if(isset($location)){
	...
	function set_level($int=0){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_level('.$int.')',__FILE__,__LINE__);}
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_lp_view('.$lp_view_id.')',__FILE__,__LINE__);}
		if(!empty($lp_view_id) and $lp_view_id = intval(strval($lp_view_id)))
		{
			$this->view_id = $lp_view_id;
			$item_view_table = Database::get_course_table(TABLE_LP_ITEM_VIEW);
			//get the lp_item_view with the highest view_count
			$sql = "SELECT * FROM $item_view_table WHERE lp_item_id = '"
					.$this->get_id()."' AND lp_view_id = ".$lp_view_id;
			 $sql .=" AND cc='".api_get_course_code()."' ";
			$sql.=" ORDER BY view_count DESC";
			if($this->debug>2){api_scorm_log('New LP - In learnpathItem::set_lp_view() - Querying lp_item_view: '.$sql,__FILE__,__LINE__);}
			$res = api_sql_query($sql,__FILE__,__LINE__);
			if(Database::num_rows($res)>0){
				$row = Database::fetch_array($res,"ASSOC");
				...
				if($this->debug>2){api_scorm_log('New LP - In learnpathItem::set_lp_view() - Updated item object with database values',__FILE__,__LINE__);}
		...
		//end
		if($this->debug>2){api_scorm_log('New LP - End of learnpathItem::set_lp_view()',__FILE__,__LINE__);}
	}
	...
	function set_path($string=''){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_path()',__FILE__,__LINE__);}
	...
	function set_prevent_reinit($prevent){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_prevent_reinit()',__FILE__,__LINE__);}
		if($prevent){
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_score('.$score.')',__FILE__,__LINE__);}
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_max_score('.$score.')',__FILE__,__LINE__);}
		if(is_int($score) or $score == '')
		{
			$this->view_max_score = Database::escape_string($score);
			if($this->debug>1){api_scorm_log('New LP - In learnpathItem::set_max_score() - Updated object score of item '.$this->db_id.' to '.$this->view_max_score,__FILE__,__LINE__);}
			return true;
		}
		return false;
	}
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_status('.$status.')',__FILE__,__LINE__);}
		$found = false;
		...
		if($found)
		{
			...
			if($this->debug>1){api_scorm_log('New LP - In learnpathItem::set_status() - Updated object status of item '.$this->db_id.' to '.$this->status,__FILE__,__LINE__);}
			return true;
		}
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_time('.$scorm_time.')',__FILE__,__LINE__);}
		if($scorm_time == 0 and ($this->type!='sco') and $this->current_start_time!=0){
			...
			if($my_time > 0){
				...
				if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_time('.$scorm_time.') - found asset - set time to '.$my_time,__FILE__,__LINE__);}
			}
		}else{
			if($format == 'scorm'){
				$res = array();
	...
	function set_title($string=''){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_title()',__FILE__,__LINE__);}
	...
	function set_type($string=''){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::set_type()',__FILE__,__LINE__);}
	...
	{
		if($this->debug>1){api_scorm_log('New LP - In learnpathItem::status_is('.print_r($list,true).') on item '.$this->db_id,__FILE__,__LINE__);}
		$mystatus = $this->get_status(true);
		if(empty($mystatus)){
			return false;
		}
		...
		foreach($list as $status)
		{
			...
			{
				if($this->debug>2){api_scorm_log('New LP - learnpathItem::status_is() - Found status '.$status.' corresponding to current status',__FILE__,__LINE__);}
				$found = true;
				return $found;
			}
		}
		if($this->debug>2){api_scorm_log('New LP - learnpathItem::status_is() - Status '.$mystatus.' did not match request',__FILE__,__LINE__);}
		return $found;
	}
	...
	function update_time($total_sec=0){
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::update_time('.$total_sec.')',__FILE__,__LINE__);}
		if($total_sec>=0){
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::write_objectives_to_db()',__FILE__,__LINE__);}
		if(is_array($this->objectives) && count($this->objectives)>0){
			...
			if(Database::num_rows($res)>0){
				$row = Database::fetch_array($res);
				$lp_iv_id = $row[0];
				if($this->debug>2){api_scorm_log('New LP - In learnpathItem::write_to_db() - Got item_view_id '.$lp_iv_id.', now checking objectives ',__FILE__,__LINE__);}
				foreach($this->objectives as $index => $objective){
					...
					// $iva_sql .=" AND cc='".api_get_course_code()."' ";
					...
					if(Database::num_rows($iva_res)>0){
						...
						$ivau_sql = "UPDATE $iva_table " .
	     					...
	     					"WHERE id = $iva_id";
						 $ivau_sql .=" AND cc='".api_get_course_code()."' ";
						...
						$sql_data=array("lp_iv_id"=>$lp_iv_id,"order_id"=>$index,"objective_id"=>$objective[0],
									"status"=>$objective[1],"score_raw"=>$objective[2],"score_min"=>$objective[4],
									"score_max"=>$objective[3]);
						 $sql_data['cc']=api_get_course_code();
						$ivai_sql=Database::sql_insert($iva_table,$sql_data);
	...
	{
		if($this->debug>0){api_scorm_log('New LP - In learnpathItem::write_to_db()',__FILE__,__LINE__);}
		...
		if (in_array($row_verified['status'],$my_case_completed)) {//liyu: 20091204
			$save=false;
		} else {
			$save=true;
		}

		if (($save===false && $this->type == 'sco') ||(($this->type == 'sco')
			&& ($credit == 'no-credit' OR $mode == 'review' OR $mode == 'browse')))	{
			//this info shouldn't be saved as the credit or lesson mode info prevent it
			if($this->debug>1){api_scorm_log('New LP - In learnpathItem::write_to_db() - credit('.$credit.') or lesson_mode('.$mode.') prevent recording!',__FILE__,__LINE__);}
		} else {
			//check the row exists
			$inserted = false;

			// this a special case for multiple attempts and ZLMS exercises
			if ($this->type == 'quiz' && $this->get_prevent_reinit()==0 && $this->get_status()=='completed') {
				// we force the item to be restarted
				$this->restart();
											
				$sql_data=array("total_time"=>$this->get_total_time(), "start_time"=>$this->current_start_time,
				"score"=>$this->get_score(), "status"=>$this->get_status(false), "max_score"=>$this->get_max(),
				"lp_item_id"=>$this->db_id,	"lp_view_id"=>$this->view_id, "view_count"=>$this->get_attempt_id(),
				"suspend_data"=>$this->current_data,"lesson_location"=>$this->lesson_location,
				/*"max_time_allowed"=>$this->get_max_time_allowed()*/);
				 $sql_data['cc']=api_get_course_code();
				$sql=Database::sql_insert($item_view_table,$sql_data);

				if($this->debug>2){api_scorm_log('New LP - In learnpathItem::write_to_db() - Inserting into item_view forced: '.$sql,__FILE__,__LINE__);}
				$res = api_sql_query($sql,__FILE__,__LINE__);
			...
			$item_view_table = Database::get_course_table(TABLE_LP_ITEM_VIEW);
			$check = "SELECT * FROM $item_view_table WHERE lp_item_id = ".$this->db_id. " " .
	     			"AND   lp_view_id = '".$this->view_id. "' AND   view_count = '".$this->get_attempt_id()."'";
			 $check .=" AND cc='".api_get_course_code()."' ";
			if($this->debug>2){api_scorm_log('New LP - In learnpathItem::write_to_db() - Querying item_view: '.$check,__FILE__,__LINE__);}
			$check_res = api_sql_query($check,__FILE__,__LINE__);
			...
			if( $inserted==false && Database::num_rows($check_res)<1){
				...
				"suspend_data"=>$this->current_data);
				 $sql_data['cc']=api_get_course_code();
				$sql=Database::sql_insert($item_view_table,$sql_data);
				if($this->debug>2){api_scorm_log('New LP - In learnpathItem::write_to_db() - Inserting into item_view: '.$sql,__FILE__,__LINE__);}
				$res = api_sql_query($sql,__FILE__,__LINE__);
				$this->db_item_view_id = Database::get_last_insert_id();
			} else {
				$sql = '';
				if($this->type=='hotpotatoes') {
					...
					$sql = "UPDATE $item_view_table " .
			     			...
			     			"WHERE lp_item_id = ".$this->db_id." " .
			     			"AND lp_view_id = ".$this->view_id." " .
			     			"AND view_count = ".$this->attempt_id." AND cc='".api_get_course_code()."'";
				} else {
					...
					if ($this->type=='quiz') {
						...
						if ($this->get_prevent_reinit()==1) {
							...
							//get type lp: 1=lp ZLMS and  2=scorm
					...
					//$sql_data=array("start_time"=>$this->get_current_start_time(),"score"=>$this->get_score(),);
					//$sql=Database::sql_update($item_view_table,$sql_data);

					$sql = "UPDATE $item_view_table  SET " .$total_time.
			     			" start_time = ".$this->get_current_start_time().", " .
			     			" score = ".$this->get_score().", " . $my_status.
			     			" max_score = '".$this->get_max()."'," .
			     			" suspend_data = '".Database::escape_string($this->current_data)."'," .
							//" max_time_allowed = '".$this->get_max_time_allowed()."'," .
			     			" lesson_location = '".$this->lesson_location."', core_exit='" .$this->core_exit."'
			     			 WHERE lp_item_id = ".$this->db_id." " .
			     			"AND lp_view_id = ".$this->view_id." " .
			     			"AND view_count = ".$this->attempt_id." AND cc='".api_get_course_code()."'";
				...
				if($this->debug>2){api_scorm_log('New LP - In learnpathItem::write_to_db() - Updating item_view: '.$sql,__FILE__,__LINE__);}
				$res = api_sql_query($sql,__FILE__,__LINE__);
			}
			//if(!$res)
			//{
			...
			if(is_array($this->interactions) && count($this->interactions)>0){
				...
				if(Database::num_rows($res)>0){
					$row = Database::fetch_array($res);
					$lp_iv_id = $row[0];
					if($this->debug>2){api_scorm_log('New LP - In learnpathItem::write_to_db() - Got item_view_id '.$lp_iv_id.', now checking interactions ',__FILE__,__LINE__);}
					foreach($this->interactions as $index => $interaction){
						...
						if(Database::num_rows($iva_res)>0){
							...
							$ivai_sql = "INSERT INTO $iva_table " .
		     						...
		     						"student_response, result, latency,cc)" .
		     						"VALUES" .
		...
		if($this->debug>2){api_scorm_log('New LP - End of learnpathItem::write_to_db()',__FILE__,__LINE__);}
		return true;
	}
}
?>
