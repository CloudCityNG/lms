...
$_SESSION['lesson_id'] =getgpc('cidReq','G');
//echo $_SESSION['lesson_id'],'<br/>';
//echo $lessonid;
...
//echo $step_num;
//$s_sql="SELECT max(id) FROM  $tbl_step ";
//$max = Database::getval( $s_sql, __FILE__, __LINE__ );
...
function setImagePreview() {
        ...
        var imgObjPreview=document.getElementById("preview");
                if(docObj.files &&    docObj.files[0]){
                        ...
                        imgObjPreview.style.width = "802px";
                        imgObjPreview.style.height = "603px";
      ...
      imgObjPreview.src = window.URL.createObjectURL(docObj.files[0]);
                        ...
                        //必须设置初始大小
                        localImagId.style.width = "802px";
                        localImagId.style.height = "603px";
...
//    ev = ev || window.event;
//    var mousePos = mousePosition(ev);
//    document.getElementById("x2").value = mousePos.x;
//document.getElementById("y2").value = mousePos.y;
//}
...
$form = new FormValidator ( 'course_step', 'POST', 'step_add.php?cidReq=' . $lessonid, '' );
...
//var_dump($_FILES);
    ...
    move_uploaded_file($image_url,"$dir/www/lms/storage/courses/$lessonid/document/images/$pname.png");
...
//    $step_answer['right4'] = $step['right4'];
    ...
    $step_hot_zone['xxx'] = $step['title_hot_x']-140;
    $step_hot_zone['yyy'] = $step['title_hot_y'];
    $action_hot_zone['x1'] = $step['action_hot_x1']-140;
    $action_hot_zone['y1'] = $step['action_hot_y1'];
    $action_hot_zone['x2'] = $step['action_hot_x2']-140;
    $action_hot_zone['y2'] = $step['action_hot_y2'];
    ...
    $sql = Database::sql_insert ( "simulation", $sql_data );
    $result = api_sql_query ( $sql, __FILE__, __LINE__ );

//    $log_msg = get_lang ( 'AddCourseCateogry' ) . "code=" . $_POST ['categoryCode'] . ",name=" . $_POST ['categoryName'];
//    api_logging ( $log_msg, 'EXTENSIONS', 'AddCourseCateogry' );
    //zhangshaolang
    exec("mysql -uroot vslab -e 'alter table simulation order by step_id ASC'");

    if(isset($step['submit_plus'])){
        $redirect_url = "step_add.php?cidReq=" . $_SESSION['_cid'];
        echo "<script>document.location='{$redirect_url}'</script>";

    }
    $redirect_url = "../../main/course_description/step.php?cidReq=" . $lessonid;
