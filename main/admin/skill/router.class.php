...
function loading_labs($USERID,$occupation_id){
    create_run_device();
    if(!$USERID){
    ...
    $urlhome='/tmp/mnt/iostmp/'.$USERID;
     ...
     if(file_exists($urlhome."/".$occupation_id)){
         exec("sudo -u root rm -r  ".$urlhome."/".$occupation_id);
     } 
    ...
    $url_routercourse=glob(URL_ROOT."/www".URL_APPEDND."/storage/routecourses/".$occupation_id."-*.lib");
    $url_router=$url_routercourse[0];
    $url_user=$url.$USERID."/".$occupation_id; 
    
    $sql1="select `id`,`skill_name`,`netMap`,`devices_offset` from `skill_occupation` where `id`=".$occupation_id;
    $topo_data= api_sql_query_array_assoc($sql1,__FILE__,__LINE__);
    $id     =$topo_data[0]['id'];
    $name   =$topo_data[0]['skill_name'];
    $netmap =$topo_data[0]['netMap'];
    $diagram=$topo_data[0]['devices_offset'];
    ...
    $net_dev_ids='';
    foreach ($netmap5  as  $val){
        $d_p=explode("A", $val);
        $net_dev_id=$d_p[0];
        $net_dev_ids[]=$net_dev_id;
        $net_dev_type= DATABASE::getval("select  `vm_type`  from  `skill_occupation_vmdisk`  where  `vm_id`=".$net_dev_id."  and  `occupat_id`='".$occupation_id."'");
        if($net_dev_type==2){
            $netmap5_new[]=$val;
        }
    }
   
    $diagram_arr=  unserialize($diagram);
 
    foreach ($diagram_arr  as  $key=>$value){
        if(in_array($key,$net_dev_ids)){
            
        }else{
            $net_dev_type= DATABASE::getval("select  `vm_type`  from  `skill_occupation_vmdisk`  where  `vm_id`=".$key."  and  `occupat_id`='".$occupation_id."'");
            if($net_dev_type==2){
                $netmap5_new[]=$key."A0B0";
            }
        } 
    }
 

    foreach($netmap5_new as $kk=>$net){
    ...
    $sql3 = "select * FROM  `net_devices` where `status`='".$occupation_id."'";
    ...
    for($j=0;$j<count($devices_data);$j++){
        ...
        $sql_count="select count(*) from `labs_run_devices` where `course_name`='".$id."' and `DEVICEID` ='".$DEVICEID."' and `USERID`='".$USERID."'";
        $devices=DATABASE::getval($sql_count,__FILE__,__LINE__);

        if($devices==0){
            $run_device_sql = "INSERT INTO `labs_run_devices` (`id`, `course_name`, `p_id`, `USERID`, `PORT`, `DEVICEID`, `ROUTETYPE`, `ROUTEMOD`, `DEVICEDTYPE`,`DEVICEDNAME`,`status`) VALUES(NULL,'".$id."','".$p_id."','".$USERID."','".$PORT."','".$DEVICEID."','".$ROUTETYPE."','".$ROUTEMOD."','".$DEVICEDTYPE."','".$DEVICEDNAME."','1');";
            @api_sql_query($run_device_sql,__FILE__,__LINE__);
        }else{
            $delete_port_sql="UPDATE  `labs_run_devices` SET  `p_id` =  '".$p_id."',`PORT`='".$PORT."',`ROUTETYPE`='".$ROUTETYPE."',`ROUTEMOD`='".$ROUTEMOD."',`DEVICEDTYPE`='".$DEVICEDTYPE."',`status`='1' WHERE `course_name`='".$id."' and `DEVICEID` ='".$DEVICEID."' and `USERID`='".$USERID."'";
    	...
    	$sql_vmdisks="select `vmdisks` from `net_devices` where `name`='".$DEVICEDNAME."' and `status`='".$name."'" ;
        $sql_systemmod= DATABASE::getval($sql_vmdisks,__FILE__,__LINE__);
        
        $command='sudo -u root /sbin/cloudvmroute.sh  system='.$sql_systemmod.'___LABSNAME='.$occupation_id.'___USERID='.$USERID.'___PORT='.$PORT.'___DEVICEDTYPE='.$DEVICEDTYPE.'___DEVICEDNAME='.$DEVICEDNAME.'___DEVICEID='.$DEVICEID.'___ROUTETYPE='.$ROUTETYPE.'___IOSFILENAME='.$IOSFILENAME.'___ROUTEMOD='.$ROUTEMOD.'___MEM='.$MEM.'___NVRAM='.$NVRAM.'___idlepc='.$idlepc.'___LINKDATA='.$LINKDATA.'__slot='.$slot;
        $command =  str_replace("____", "___", $command);
        exec("echo $command '&' >> /tmp/www/$USERID'_'$occupation_id'.sh' ; chmod 777 /tmp/www/$USERID'_'$occupation_id'.sh' ");
        
    }
	$tophome = $urlhome."/".$occupation_id;
        if(!file_exists($tophome)){
            exec(" mkdir -p ".$urlhome);
		if(!empty($url_router)){
			$exec_var0="cd ".$urlhome."/;sudo -u root  tar -zxf ".$url_router." ".$occupation_id;
			exec($exec_var0);
		}
	}
    
        exec("/tmp/www/$USERID'_'$occupation_id'.sh' > /dev/null & ");
        exec("sleep 2 ;rm -rf  /tmp/www/$USERID'_'$occupation_id'.sh'");
    
    $uport_sql="UPDATE   `labs_run_devices` SET  `uport` = '".$uporta."'  WHERE  `USERID` ='".$USERID."' and  `course_name`='".$id."'";
    @api_sql_query($uport_sql,__FILE__,__LINE__);
}

function stoping_labs($USERID,$occupation_id){
    if(!$USERID){
        $USERID=$_SESSION['_user']['user_id'];
    }
     
    $stop_sql="select `id`,`USERID`,`DEVICEID`, `DEVICEDTYPE` , `PORT`,`uport` from `labs_run_devices` where `course_name`='".$occupation_id."' and `USERID`=".$USERID;
    ...
    foreach($running_data as $k=>$v){
        ...
        if($d_id){
            $delete_sql="DELETE FROM `labs_run_devices` WHERE  `id` = ".$d_id;
            @api_sql_query($delete_sql,__FILE__,__LINE__);
        }

        $deletecommand="sudo -u root /sbin/cloudvmroutestop.sh  ".$userId." ".$occupation_id." ".$userId."_".$occupation_id."_".$DEVICEDTYPE."_".$deviceId." ".$userId."_".$occupation_id."_".$PORT;
