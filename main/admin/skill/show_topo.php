...
$title=api_get_setting('siteName');
 ///////main/admin/skill/show_topo.php?action=show&occupation_id=3
$action=  getgpc("action");
$occupation_id= intval(getgpc('occupation_id')); 
 $status=  getgpc("status");
 $USERID=$_SESSION['_user']['user_id'];

 //loading  topo
 if($status=="loading"){   
     $ccode=getgpc('occupation_id'); 
    loading_labs($USERID,$ccode);
     header("Location: ./show_topo.php?action=show&occupation_id=".$occupation_id."&status=load");
 }
 //stoping  topo
  if($status=="stoping"){   
    stoping_labs($USERID,getgpc('occupation_id'));
     header("Location: ./show_topo.php?action=show&occupation_id=".$occupation_id."&status=stop");
 }
 //load<--->stop 
 if($status=="load"){
     $sta="stoping";
...
if($action=="show" &&  $occupation_id){
    $sql_offset="select  `devices_offset`  from  `skill_occupation`  where  `id`=".$occupation_id;
    $dev_offset= api_sql_query_array_assoc($sql_offset,__FILE__,__LINE__);       
    $devices_offset=unserialize($dev_offset[0]['devices_offset']); 
    $dev_str='{"nodes":[';
 ////type=1模板，一模板一设备
    $sql="select `vm_id`  from  `skill_occupation_vmdisk` where  `vm_type`=1  and  `occupat_id`= ".$occupation_id; 
    $vms= api_sql_query_array_assoc($sql,__FILE__,__LINE__);    
    foreach ($vms  as  $vm){   
       $sql="select  *  from  `vmdisk`  where  `id`=". $vm['vm_id'];
       ...
       $desc_left="-40";
       $desc_top="120";
       $netmap=DATABASE::getval("select  `netMap`  from  `skill_occupation`  where  `id`=".$occupation_id);
  ...
  ////type=2路由
    $sql="select `vm_id`  from  `skill_occupation_vmdisk` where  `vm_type`=2  and  `occupat_id`= ".$occupation_id; 
    $routers= api_sql_query_array_assoc($sql,__FILE__,__LINE__);     
    foreach ($routers  as  $router){     
        $sql_dev="select  *  from  `net_devices`  where   `id`=".$router['vm_id'];
...
<head>
    ...
    <link href="../../../netMap/css/template_netmap.css" rel="stylesheet" type="text/css"/>
    <link href="../../../netMap/css/jquery.contextMenu.css" rel="stylesheet" type="text/css"/>
    <link rel="stylesheet" media="all" type="text/css" href="../../../netMap/css/jquery-impromptu.css"/>
    ...
    <script type='text/javascript' src='../../../netMap/js/jquery-1.8.2.min.js'></script>
    <script type='text/javascript' src='../../../netMap/js/jquery-ui-1.8.23.custom.min.js'></script>
    <script type='text/javascript' src='../../../netMap/js/jquery.jsPlumb-1.3.14-all-min.js'></script>
    <script type="text/javascript" src="../../../netMap/js/jquery.contextMenu.js"></script>
    <script type="text/javascript" src="../../../netMap/js/HashMap.js"></script>
    <script type="text/javascript" src="../../../netMap/js/jquery.jsIOUWebNetMap.js"></script>
    <script>
...
//        });
        ...
        deviceObj.bind("dblclick", function(e) {  
            ...
            if(device.type=='服务器' || device.type=='客户端' || device.type=='防火墙' || device.type=='局域网'){  
                window.open ('http://<?=$local_addresd.URL_APPEDND ?>/main/cloud/cloudvmstart.php?system='+ device.nodeName+'&nicnum=1&occupation_id=<?=$occupation_id?>',"_blank" ) ;
            }else{  
                window.open ('http://<?=$local_addresd.URL_APPEDND ?>/main/admin/skill/connectDevice.php?nodesId='+consoleId+'&occupation_id=<?=$occupation_id?>',"_blank" ) ;
            }
        //            e.preventDefault();
...
//alert( JSON.stringify(json.nodes));
    ...
    <script type="text/javascript" src="../../../netMap/js/jquery-impromptu.js"></script>
...
//            document.cookie="c_code=<?=$occupation_id?>";
//            window.location.href="topoSave.php";
//        }
//        function cleanMap() {
//            document.cookie="actionss=cleanMap";
//            document.cookie="cid=<?=$occupation_id?>";
...
//            alert(jsonStr);
            ...
            sidebarHtml += '<span class="title"><?=$toponame?></span><br>';
              sidebarHtml += '<a  class= "action_botton" href="show_topo.php?status=<?=$sta?>&occupation_id=<?=$occupation_id?>"   style="color:red"><img  width="15" height="15" src="../../../topoDesign/images/<?=$sta_img?>"><?=$sta_words?><\/a>';
