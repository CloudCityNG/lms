...
if($id){
   ...
   if($id){
           if($devices_offset){
               $sql1="UPDATE  `".DB_NAME."`.`skill_occupation` SET `devices_offset` =  '".$devices_offset."'  WHERE  `id` =".$id;
               $r1= @api_sql_query($sql1,__FILE__,__LINE__);
 
           }
           
            $sql="UPDATE  `".DB_NAME."`.`skill_occupation` SET  `netMap` =  '".$net_var."'  WHERE  `id` =".$id;
            $r2= @api_sql_query($sql,__FILE__,__LINE__);
        }
   api_redirect("design_topo.php?action=design&occupation_id=".$id);
}else{
