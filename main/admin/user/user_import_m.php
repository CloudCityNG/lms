...
function complete_missing_data($users, $import_dept = 0) {
	foreach ( $users as $index => $user ) {
		...
		if (! isset ( $login_name ) || strlen ( $login_name ) == 0) {
			$users [$index] ['UserName'] = $user['FirstName'];
		}
		...
		if (! isset ( $user ['AuthSource'] ) || strlen ( $user ['AuthSource'] ) == 0) {
  		...
  		if (empty ( $import_dept )) { //使用文件中部门                    
			if (is_not_blank ( $user ['DeptNo'] )) {
				...
				$users [$index] ['DeptID'] = DATABASE::getval ( $sql );
			} else {
...
function save_data($users) {
	...
	foreach ( $users as $index => $user ) {
		...
		if (UserManager::is_username_available ( $username )) { //用户名不存在，即用户可用时
			$user_id = UserManager::create_user (
                                                 $user ["姓名"],$user ["职务"],5,$user ["战队ID"],$user ["邮箱"],
                                                 $username,$pass, $user ["工号，学号，编号"],'0', $platformLanguage,$user ["固话"],
...
if (count ( $errors ) > 0) {
	...
	foreach ( $errors as $index => $error_user ) {
		$error_message .= '<li>' . sprintf ( get_lang ( "LineN" ), $index ) . ":&nbsp;" . $error_user ['UserName'] . ' -- ' . $error_user ['FirstName'] . ': ';
