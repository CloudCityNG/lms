    ...
    switch ($action) {
        case 'delete' :
            ...
            if ( isset($delete_id)){
                ...
                $sql = "DELETE FROM  `setup` WHERE `setup`.`id` = {$delete_id}";
...
function get_labs_topo_data($from, $number_of_items, $column, $direction) {
    ...
    $sql = "select `id`,`custom_number`,`title`,`description`,`id`,`id`,`id` FROM ".$labs_topo." WHERE 1 ";
...
$table->set_header ( $idx ++, '自定义编号', false, null,array ('style' => 'width:10%' ));
