...
// Predefined date formats in ZLMS provided by the language sub-system.
...
/**
 * Whenever the server type in the ZLMS Config settings is
 ...
 * 2. Whenever the server type in the ZLMS Config settings is set to
 ...
 * 3. DLTT means ZLMS Language Translation Tool.
 * @link http://www.ZLMS.com/DLTT/
 */
function _get_lang($variable, $notrans = 'DLTT', $language = null) {
	...
	// This is a flag for showing the link to the ZLMS Language Translation Tool
	...
	$cache[$language][$dltt][$variable] =
		$ot.$variable.$ct."<a href=\"http://www.ZLMS.com/DLTT/suggestion.php?file=".$language_file.".inc.php&amp;variable=$".$variable."&amp;language=".$language_interface."\" target=\"_blank\" style=\"color:#FF0000\"><strong>#</strong></a>";
	return $cache[$language][$dltt][$variable];
}
 ...
 */
function _api_get_interface_language($purified = false) {
	global 	$language_interface;
 ...
 * formats in the ZLMS system: TIME_NO_SEC_FORMAT, DATE_FORMAT_SHORT, DATE_FORMAT_LONG, DATE_TIME_FORMAT_LONG.
 ...
 * Note: See the file ZLMS/main/inc/lib/internationalization_database/name_order_conventions.php where you can revise the convention for your language.
 * @author Carlos Vargas <carlos.vargas@ZLMS.com> - initial implementation.
 * @author Ivan Tcholakov
 */
