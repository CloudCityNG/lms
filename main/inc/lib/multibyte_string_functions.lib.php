 ...
 *
 * Adaptation for the ZLMS 1.8.6.1, 12-JUN-2009:
 ...
 * @author Ivan Tcholakov, August 2009, adaptation for the ZLMS LMS.
 */
...
function api_get_file_system_encoding() {
	...
	if (!isset($file_system_encoding)) {
		$locale = setlocale(LC_CTYPE, '0'); //Chinese_People's Republic of China.936
		$seek_pos = strpos($locale, '.');
		if ($seek_pos !== false) {
			$file_system_encoding = substr($locale, $seek_pos + 1);
			if (IS_WINDOWS_OS) {
				$file_system_encoding = 'CP'.$file_system_encoding;  //CP936
			}
		}
 ...
 * Note: See the file ZLMS/main/inc/lib/internationalization_database/non_utf8_encodings.php
