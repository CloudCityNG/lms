...
class ezSQL_mysql extends ezSQLcore {
	...
	function ezSQL_mysql($dbuser = '', $dbpassword = '', $dbname = '', $dbhost = 'localhost', $dbport = '3306') {
		$this->dbuser = $dbuser;
	...
	function quick_connect($dbuser = '', $dbpassword = '', $dbname = '', $dbhost = 'localhost', $dbport = '3306') {
		$return_val = false;
		if (! $this->connect ( $dbuser, $dbpassword, $dbhost, $dbport, true ))
			;
	...
	function connect($dbuser = '', $dbpassword = '', $dbhost = 'localhost', $dbport = '3306') {
		...
		else if (! $this->dbh = @mysql_connect ( $dbhost . ':' . $dbport, $dbuser, $dbpassword, true )) {
	...
	function select($dbname = '') {
		...
		// Try to connect to the database
			...
			if (! $str = @mysql_error ( $this->dbh )) $str = $ezsql_mysql_str [5];
	...
	function query($query) {
		...
		if ($cache = $this->get_cache ( $query )) {return $cache;}
