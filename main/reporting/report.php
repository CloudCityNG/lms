...
function content_filter($id){
    $result = "";
    $comment=Database::getval('select `screenshot_file` from `report` where `id`='.urlencode($id),__FILE__,__LINE__);
    $result.="<a href='report.php?action=view&id=".$id."'><span style='text-align:left'>".$comment."</span></a>";
    return $result;
}
...
function return_filter($id){
    $result = "";
    $marking_status=Database::getval("select `marking_status` from `vslab`.`report` where `id`=".$id,__FILE__,__LINE__);
    $return=Database::getval("select `return` from `vslab`.`report` where `id`=".$id,__FILE__,__LINE__);
    if($marking_status==1){
        if($return==1){
            $result.=' 通过';
        }else{
...
function action_filter($id){
    ...
    $marking_status =Database::getval("select `marking_status` from `vslab`.`report` where `id`=".$id,__FILE__,__LINE__);
    if($marking_status==0){
        $result .=  link_button ( 'plugin.gif', '批改实验报告', 'report_edit.php?id=' . $id, '100%', '70%', FALSE );
    }
    ...
    switch ($_POST ['action']) {
        case 'deletes' :
            ...
            foreach ( $_POST ['id'] as $index => $id ) {
                $sql = "DELETE FROM `vslab`.`report` WHERE id='" . $id . "'";
...
function get_report_data($from, $number_of_items, $column, $direction) {
    $report = Database::get_main_table ( report );
    $sql = "select `id`,`report_name`,`user`,`id`,`key`,`score`,`comment`,`id` ,`id` FROM  ".$report." where `status`=1 ";
...
//            if($report_count > 0){
//                echo '&nbsp;&nbsp;' . link_button ( 'return.gif', '导出实验报告', 'report_export.php', '60%', '50%' );
//            }
            ?>
        </span>
    </div>
