...
$sql = "SELECT id,title,type,active,description,max_attempt,max_duration,cc,created_user FROM $TBL_EXERCICES AS ce
			WHERE active<>'-1' AND type='" . escape ( $type ). "' AND title like '%".$keyword."%' ORDER BY display_order";
}else{
   $sql = "SELECT id,title,type,active,description,max_attempt,max_duration,cc,created_user FROM $TBL_EXERCICES AS ce
...
$query_vars = array ();
if (is_not_blank ( $_REQUEST ['keyword'] )) $query_vars ['keyword'] = getgpc ( 'keyword','G' );
