...
function save_data($data, $cc, $pool_id) {
	if (empty ( $data )) return false;
		...
		foreach ( $data as $key => $item ) {
			...
			if (in_array ( $item ["type"], array (UNIQUE_ANSWER, MULTIPLE_ANSWER, TRUE_FALSE_ANSWER, COMBAT_QUESTION ) )) {
...
function parse_upload_data($data_rows) {
	...
	if (is_array ( $data_rows ) && count ( $data_rows ) > 0) {
		foreach ( $data_rows as $key => $item ) {
			...
			} elseif ($question_type == $_question_types[COMBAT_QUESTION]) {
                $questionItem ['type'] = COMBAT_QUESTION;
            }
			$question_score = floatval ( $item ['题目分数'] );
			$questionItem ['ponderation'] = floatval ( $question_score ); //分数
			$questionItem ['level'] = $item ['题目难度']; //难度
			$questionItem ['question'] = trim ( $item ['试题内容'] ); //试题内容
			$questionItem ['comment'] = trim ( $item ['题目解析'] );//题目解析
			$questionItem ['question_code'] = (empty ( $item ['课程编号'] ) ? Question::_get_question_code () : $item ['课程编号']);
			...
			$available_question_type = array (UNIQUE_ANSWER, MULTIPLE_ANSWER, TRUE_FALSE_ANSWER, COMBAT_QUESTION);
			...
			if (empty ( $item ["QuestionTrunkLineNumber"] ) && in_array ( $questionItem ['type'], $available_question_type )) {
				...
				if ($questionItem ['type'] == UNIQUE_ANSWER or $questionItem ['type'] == MULTIPLE_ANSWER) { //选择题
					...
					$options = array_values(array_slice($item,7,16));
				...
				} elseif ($questionItem ['type'] == COMBAT_QUESTION){

                        $questionItem ['is_k'] = $item['是否有key值'] == '是' ? 1 : 0;
                        $questionItem ['is_up'] = $item['是否有报告'];
                        $questionItem ['vm_name'] = $item['科目场景'];
                        if($questionItem ['is_k'] == 1){
                            $key_arr = array();
                            $key_score = array_values(array_slice($item,20));
                            $keys_arr = array();
                            $score_arr = array();
