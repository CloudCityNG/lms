...
if (isset ( $_POST ['action'] ) && is_equal ( $_POST ["action"], "changeOrders" )) {
	...
	$questionOrders =$_POST['qo'];     
	$questionScores =$_POST['qs'];
    $same_score=$_POST['same_score'];
	if ($questionOrders && is_array ( $questionOrders )) {            
		foreach ( $questionOrders as $question_id => $question_order ) {
            $question_scores = ($same_score || $same_score === '0') ? $same_score : $questionScores [$question_id];
			$sql_data = array ("question_order" => intval($question_order), 'question_score' => $question_scores);
...
//ALTER TABLE  `exam_rel_question` CHANGE  `question_score`  `question_score` FLOAT NULL DEFAULT  '1'
		if ($exerciseId) {
...
if ($objExercise->active == 0) { //没有发布时
	...
	echo link_button ( 'database.gif', 'ChoseFromQuestionPool', api_get_path ( WEB_CODE_PATH ) . 'exercice/question_pool.php?fromExercise=' . $exerciseId, '80%', '90%' );
...
if ($nbrQuestions) {
	...
	foreach ( $questionList as $pos => $id ) {
		...
		if (is_object ( $objQuestionTmp )) {
			...
			if ($type == $strActionType) {
				...
				//分值
				$tmp_weight = ($id ['question_score'] || $id ['question_score'] === '0') ? $id ['question_score'] : $objQuestionTmp->weighting;
				if ($objExercise->active == 0) {
					$row [] = '<div class="edit" id="qs' . $pos . '" ><input type="text" value="' . ($tmp_weight) . '"
					class="inputText qs" name="qs[' . $pos . ']" style="width:30px;text-align:right;" onFocus="this.select();" pattern="^\d+(\.\d+)?$"  rel="qs"/></div>';
				} else {
					$row [] = $tmp_weight;
				}
				$url_quiz='./question_update.php?action=edit&qid='.$pos.'&answerType='.$id['type'].'&KeepThis=true&TB_iframe=true&modal=true&width=90%&height=80%';
				$row [] = '<a class="thickbox" href="'.$url_quiz.'">'.$i . '. ' . api_trunc_str2 ( strip_tags ( $objQuestionTmp->selectTitle () ), 50 ).'</a>';
    ...
    if ($objExercise->active == 0 && $nbrQuestions) {
        echo '<difconfig
iv style="margin:4px 0; float:left;"><button class="save" type="submit" onclick="return sub();" name="save_question"
		 id="save_question">' . get_lang('SaveQuestoinOrder') . '</button></div>';
    }
    if(intval($_GET['qtype']) !== COMBAT_QUESTION && (count($quiz_question_type) !==1 || !$quiz_question_type[COMBAT_QUESTION]) ) {
        if ($objExercise->active == 0 && $nbrQuestions) {
            echo '<div style="margin:4px 10px; float:left;">', '统一设置本题型分值为', '<input type="text" id="same_score" pattern="^\d+(\.\d+)?$" name="same_score" value="" onfocus="this.select();" class="inputText" style="width:40px">', '<input class="cancel" type="submit" name="save_score"
