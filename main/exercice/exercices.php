...
if (isset($exerciseId) && $_GET['choice']=='able') {
	...
	//$exerciseTitle = Database::get_scalar_value ( $sql );
                ...
                tb_close("exercices.php?type=$type");
}
...
$sql = "SELECT id,title,type,active,description,max_attempt,max_duration,cc,id,created_user FROM $TBL_EXERCICES AS ce
        WHERE created_user ='".$userid."' and active<>'-1' ".($type==''?'':"and type=".escape ( $type ));
}
 else {
  $sql = "SELECT id,title,type,active,description,max_attempt,max_duration,cc,id,created_user FROM $TBL_EXERCICES AS ce
...
//		$visible_html = '<a href="exercices.php?choice=able&exerciseId='.$row ['id'].'>'.Display::return_icon ( 'right.gif', get_lang ( 'QuizPublished' ) ).'</a>';
        $visible_html = '<a href="exercices.php?type='.$type.'&choice=able&exerciseId=' . $row ['id'] . '" onclick="return confirm(\'' . get_lang ( "注意：您确定要关闭这份考试试卷吗?" ) . '\');">' .Display::return_icon ( 'right.gif', get_lang ( '点击关闭考试' ) ) . '</a>';

    } else {
    
    $visible_html = '<a href="exercices.php?type='.$type.'&choice=enable&exerciseId=' . $row ['id'] . '" onclick="return confirm(\'' . get_lang ( "QuizPublishConfirm" ) . '\');">' .Display::return_icon ( 'wrong.gif', get_lang ( 'ClickToPublishQuiz' ) ) . '</a>';
	}
