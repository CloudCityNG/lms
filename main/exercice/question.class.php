...
abstract class Question {
	...
	static function read($id) {
		...
		if ($object = Database::fetch_object ( $result )) {
            ...
            $objQuestion->contents = $object->contents;//20130712
			$objQuestion->description = $object->description;
			$objQuestion->comment = $object->comment;
			$objQuestion->weighting = $object->ponderation;
			$objQuestion->vm_name = $object->vm_name;
            $objQuestion->vm_ip=$object->vm_ip;
            $objQuestion->is_up = $object->is_up;
            $objQuestion->is_k=$object->is_k;
            $objQuestion->keyss=$object->keyss;
            $objQuestion->key_score=$object->key_score;
            $objQuestion->console=$object->console;
	...
	function save($exerciseId = 0) {
        ...
        $contents = ($this->contents);//20130712
        ...
        $key_score = serialize($_POST['key_score']);
		...
		if ($id) {
                ...
                'question_code' => $question_code, 'level' => $level,'contents' => $contents,  );
                    }else{
            ...
            $sql_data['vm_ip']=  $this->vm_ip;
            $sql_data['question_code']= $question_code;
            $sql_data ['is_up'] = $this->is_up;
            $sql_data['is_k']=$this->is_k;
            $sql_data['keyss']= serialize($_POST['keyss']);
            $sql_data['key_score']= serialize($_POST['key_score']);
            $sql_data ['console']=$this->console;
            $sql_data ['answer'] = $this->answer_txt;
            $sql_data ['contents'] = $this->contents;//20130712
			...
			$sql_data = array ('question' => $question,'contents' =>$contents, 'description' => $description, 'comment' => $comment, 'ponderation' => $weighting, 'position' => $position, 'type' => $type, 'picture' => $picture,
...
function delete($deleteFromEx = 0) {
	...
	//	$sql = "SELECT COUNT(*) FROM $TBL_EXERCICE_QUESTION WHERE question_id='" . escape ( $id ) . "'";
		...
		//if (empty ( $deleteFromEx )) $_SESSION['_user']['status']!='1'
			//判断是否为COMBO或CLOZE题型
			...
			if (can_do_my_bo ( $created_user ) || $_SESSION['_user']['status'] == '1') {
	...
	function createForm(&$form) {
		...
		//编号---- > 改成题目 
		$test = $form->addElement ( 'text', 'question_code', $required_html . get_lang ( 'QuestionCode' ), array ("id" => "questionCode", 'class' => "inputText", 'style' => "width:40%" ) );
		$renderer->setElementTemplate ( $default_template, 'question_code' );
		if (empty ( $this->question_code )) {
			$form->addRule ( 'question_code', get_lang ( 'PleaseEnterQuestionCode' ), 'required' );
			//$form->addRule ( 'question_code', get_lang ( 'OnlyLettersAndNumbersAllowed' ), 'username' );
			$form->addRule ( 'question_code', get_lang ( 'QuestionCodeExist' ), 'callback', '_check_question_code' );
			$defaults ['question_code'] = $this->_get_question_code ();
		} else {
			$defaults ['question_code'] = $this->question_code;
			//$form->freeze ( array ("question_code" ) );
		}
        ...
        if ($GLOBALS ['hide_question_name'] == false) {
            ...
            if($answerType==10 or $answerType==6){
                $form->addElement ( 'textarea', 'contents',  "内容", array ( 'id' => 'description1', 'style' => 'width:100%;height:150px', 'wrap' => 'virtual', 'class' => 'inputText' ) );
                $renderer->setElementTemplate ( $default_template, 'contents' );
            }
 $group = array ();
            /*只有实战题才显示key值*/
            if(intval($_REQUEST['answerType']) === COMBAT_QUESTION) {
                $group [] = $form->createElement('radio', 'is_k', null, get_lang('否'), 0, array("id" => "is_key1"));
                $group [] = $form->createElement('radio', 'is_k', null, get_lang('是'), 1, array("id" => "is_key2"));
                $form->addGroup($group, "is_k", get_lang('是否有key提交'), '&nbsp;&nbsp;', false);
                $renderer->setElementTemplate($default_template, 'is_k');

                $keyss_count = count(unserialize($this->keyss));
                $nb_answers = isset ($_POST ['nb_answers']) ? $_POST ['nb_answers'] : $keyss_count;
                ...
                $html = '<tr class="containerBody">
			              ...
			              <td class="formTableTd" align="left">
				                <table class="quiz_data_table" width="100%" id="key_tab">
					                 <tr style="text-align: center;">
                                          <th>' . get_lang('key') . '</th>
						                  <th width="50%">' . get_lang('分数') . '</th>
					                 </tr>';
                ...
                if (!empty ($this->keyss)) {
                    $arr_key = unserialize($this->keyss);
                    $arr_key_score = unserialize($this->key_score);

                }

                $form->addElement('hidden', 'nb_answers', null, array('id' => 'numbers'));

                for ($i = 0; $i < $nb_answers; ++$i) {
                    if (is_array($arr_key)) {
                        $defaults ['keyss'] = $arr_key;
                        $defaults ['key_score'] = $arr_key_score;
                    };
                    $renderer = $form->defaultRenderer();
                    $renderer->setElementTemplate('<td>{element}<!-- BEGIN error --><span class="onError">{error}</span><!-- END error --></td>');

                    $form->addElement('text', 'keyss[' . $i . ']', null, array('class' => 'inputText', 'style' => "width:98%"));
                    $form->addElement('text', 'key_score[' . $i . ']', null, array('class' => 'inputText', 'style' => "width:98%", 'id' => "fenshu" . $i));
                    $form->addElement('html', '</tr>');
                }

                $form->addElement('html', '</table></td></tr>');

                $form->addElement('html', '<tr class="containerBody" id="add_sub"><td class="formLabel"></td><td class="formTableTd" align="left">');
                $form->addElement('submit', 'lessAnswers', get_lang('删除k值'), array('class' => "inputSubmit"));
                $form->addElement('submit', 'moreAnswers', get_lang('添加k值'), array('class' => "inputSubmit"));
		...
		$form->setConstants ( array ('nb_answers' => $nb_answers ) );
            ...
            $form->addRule ( 'media_file', get_lang ( 'UploadFileNameAre' ) . ' *.doc,*.docx', 'filename', '/\\.(doc|docx)$/' );
        }
...
//                $group = array ();
//                $group [] = $form->createElement ( 'radio', 'console', null, get_lang ( '否' ),0 );
//                $group [] = $form->createElement ( 'radio', 'console', null, get_lang ( '是' ),1 );
//                $form->addGroup ( $group, "console", get_lang ( '是否有控制台' ), '&nbsp;&nbsp;', false );
//                $renderer->setElementTemplate ( $default_template, 'console' );
        ...
        $defaults['vm_ip']=  empty($this->vm_ip)?getgpc('vm_ip'):  $this->vm_ip;
        $defaults ['is_up'] = empty ( $this->id ) ? 0 : $this->is_up;
        $defaults ['console'] = empty ( $this->id ) ? 0 : $this->console;

        $defaults ['cc'] = empty ( $this->cc ) ? getgpc ( 'cc' ) : $this->cc;
		$defaults ['questionName'] = $this->question;
        $defaults ['contents'] = $this->contents;
		$defaults ['questionDescription'] = $this->description;
		$defaults ['questionComment'] = $this->comment;
        $defaults ['question_code'] = $this->question_code;
        $defaults ['is_k'] =(($this->keyss)?'1':'0');
	...
	function processCreation($form, $objExercise) {
        ...
        //内容
        $this->updateContents ( trim ( $form->getSubmitValue ( 'contents' ) ) );
		...
		//编号
		$question_code = trim ( $form->getSubmitValue ( 'question_code' ) );
		$this->updateQuestionCode ( $question_code );
        ...
        $this->vm_ip=  trim($form->getSubmitValue('vm_ip'));
		$this->cc = trim ( $form->getSubmitValue ( 'cc' ) );
                $this->is_up = trim ( $form->getSubmitValue ( 'is_up' ) );
                        $this->is_k=trim($form->getSubmitValue('is_k'));
                $this->keyss=trim($form->getSubmitValue('keyss'));
                $this->console = trim ( $form->getSubmitValue ( 'console' ) );
    ...
    function selectContents() {
        $this->contents = api_parse_tex ( $this->contents );
    ...
    function updateVm_ip(){
        $this->vm_ip=$vm_ip;
    }
                function updateIsup($is_up) {
        $this->is_up = $is_up;
    }
        function updateIsk($is_k){
        $this->is_k=$is_k;
    }
    
    function updateKeyss($keyss){
        $this->keyss=$keyss;
    }
	...
	function updateQuestionCode($question_code) {
