...
<div aria-hidden="true" id="modal" class="modal hide fade" role="dialog" aria-labelledby="#myModalLabel">
 ...
 $qnid++; ?>
                <?php $tid++; ?>
                <?php $qmid++; ?>
            	<a id="sign_<?php echo $question['questionid']; ?>" href="javascript:;" onclick="javascript:gotoquestion(<?php echo $question['questionid']; ?>,<?php echo $quest; ?>);" rel="0" class="badge questionindex<?php if($this->tpl_var['sessionvars']['examsessionsign'][$question['questionid']]){ ?> signBorder<?php } ?>"><?php echo $tid; ?></a>
            	<?php } ?>
            	<?php $qrid = 0;
 ...
 $did++; ?>
                <?php $qmid++; ?>
				<a id="sign_<?php echo $data['questionid']; ?>" href="javascript:;" onclick="javascript:gotoquestion(<?php echo $data['questionid']; ?>,<?php echo $quest; ?>);" rel="0" class="badge questionindex<?php if($this->tpl_var['sessionvars']['examsessionsign'][$data['questionid']]){ ?> signBorder<?php } ?>"><?php echo $tid; ?>-<?php echo $did; ?></a>
                <?php } ?>
                <?php } ?>
            </dd>
        </dl>
...
<div class="row-fluid">
 ...
 $qid++; ?>
		    	...
		    	<?php if($this->tpl_var['sessionvars']['examsessionquestion']['questions'][$quest] || $this->tpl_var['sessionvars']['examsessionquestion']['questionrows'][$quest]){ ?>
				<?php $oid++; ?>
				<div id="panel-type-<?php echo $quest; ?>" class="questionpanel tab-pane<?php if((!$this->tpl_var['ctype'] && $qid == 1) || ($this->tpl_var['ctype'] == $quest)){ ?> active<?php } ?>">
					<ul class="breadcrumb">
						<li>
							<h5><?php echo $oid; ?>、<?php echo $this->tpl_var['questype'][$quest]['questype']; ?><?php echo $this->tpl_var['sessionvars']['examsessionsetting']['examsetting']['questype'][$quest]['describe']; ?></h5>
						</li>
					</ul>
					<?php $tid = 0; ?>
	                <?php $qnid = 0;
 ...
 $sid++; ?>
			                        ...
			                        <?php break;; ?>
			                        <?php } ?>
			                        <label class="radio inline"><input type="radio" name="question[<?php echo $question['questionid']; ?>]" rel="<?php echo $question['questionid']; ?>" value="<?php echo $so; ?>"  <?php if($question['answerhistory']){ ?><?php if($so == $question['answerhistory']){ ?><?php $jishu++; ?>checked="checked"<?php } ?><?php } ?>/><?php echo $so; ?> </label>
			                        <?php } ?>
 ...
 $sid++; ?>
			                        ...
			                        <?php break;; ?>
			                        <?php } ?>
			                        <label class="checkbox inline"><input type="checkbox" name="question[<?php echo $question['questionid']; ?>][<?php echo $key; ?>]" rel="<?php echo $question['questionid']; ?>" value="<?php echo $so; ?>"  <?php if(in_array($so,$this->tpl_var['sessionvars']['examsessionuseranswer'][$question['questionid']])){ ?>checked<?php } ?><?php if(in_array($so,$question['answerhistory'])){ ?><?php $jishu++; ?> checked="checked"<?php } ?>/><?php echo $so; ?> </label>
			                        <?php } ?>
		                        <?php } ?>
		                    </div>
							<?php } else { ?>
							...
							<div class="media-body well">
								<div class="btn-group pull-right">
				            		<?php if($qcid > 1){ ?>
				            		<a class="btn" href="javascript:;" onclick="javascript:gotoindexquestion(<?php echo $qcid - 2; ?>,<?php echo $quest; ?>);" title="上一题">上一题</a>
				            		<?php } ?>
				            		<?php if($qcid < $qmid){ ?>
				            		<a class="btn" href="javascript:;" onclick="javascript:gotoindexquestion(<?php echo $qcid; ?>,<?php echo $quest; ?>);" title="下一题">下一题</a>
									<?php } ?>
				            	</div>
			            	</div>
						</div>
 ...
 $sid++; ?>
				                        ...
				                        <?php } ?>
				                    <?php } elseif($this->tpl_var['questype'][$quest]['questchoice'] == 5){ ?>
		                        		<input type="text" class="input-xlarge" name="question[<?php echo $data['questionid']; ?>]" value="<?php echo $this->tpl_var['sessionvars']['examsessionuseranswer'][$data['questionid']]; ?>" rel="<?php echo $data['questionid']; ?>"/>
			                        <?php } else { ?>
				                        <?php $sid = 0;
 foreach($this->tpl_var['selectorder'] as $key => $so){ 
 $sid++; ?>
...
{
	var question = $(".paperexamcontent").eq(index);
...
$(document).ready(function(){
	...
	$('.yesdonumber').html($('#modal-body .badge-info').length+<?php echo $jishu; ?>);
	...
	$('#form1 :input[type=radio]').change(function(){
		$.post('index.php?exam-app-exam-questionhistory',
			{
				rel:$(this).attr('rel'),
				val:$(this).val(),
				type_:'radio'
	...
	$('#form1 :input[type=checkbox]').change(function(){
		...
		var answers = $(this).parent().parent().children();
		var str = '';
		answers.each(function(index){
			var checkbox = $(this).find('input:checkbox:first');
			var check_ = checkbox.attr('checked');
			if(check_ == 'checked'){
				str += index+'#'+checkbox.val()+',';
			}
		});
		$.post('index.php?exam-app-exam-questionhistory',
			{
