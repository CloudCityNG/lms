...
class app
{
	...
	public function __construct(&$G)
	{
		...
		{
		    if($this->ev->url('2') && !in_array($this->ev->url('2'),array('index','basics','exam','recover')) && !in_array($this->_user['sessiongroupid'], [1,9]))
		    {
	...
	public function index()
	{
		...
		switch($action)
		{
			...
			switch($this->ev->url(4))
			{
				...
				//获取剩余时间
				case 'lefttime':
				/* $sessionvars = $this->basic->getExamSessionBySessionid();
				$lefttime = TIME - $sessionvars['examsessionstarttime']; */
			    $sessionvars = $this->basic->getBasic();
			    $lefttime = TIME-$sessionvars['basicexam']['opentime']['start'];
	...
	public function exam()
	{
		...
		switch($action)
		{
			...
			case 'historyanswers':
		        ...
		        foreach($sessionvars['examsessionquestion']['questions'] as $val){
		            $key = $sessionvars['examsessionuserid'].$sessionvars['examsessionbasic'].$questionid;
		            $this->basic->addQuestionHistory(array('keystring'=>strval($key),'uid'=>intval($sessionvars['examsessionuserid']),'answer'=>$answer),true);
			    }
			...
			case 'questionhistory':
			    ...
			    if($type && $answerPost && $questionid){
			        if($type == 'radio'){
			            $answer = json_encode([$questionid=>$answerPost]);
			        }
			        if($type == 'checkbox'){
			            $answerPost = substr($answerPost,0,-1);
			            $answerTmp = explode(',',$answerPost);
			            foreach($answerTmp as $v){
			                $valtemp = explode('#',$v);
			                $answer[$valtemp[0]] = $valtemp[1];
			            }
			            $answer = json_encode([$questionid=>$answer]);
			        }
			    }
			    
	            $key = $sessionvars['examsessionuserid'].$sessionvars['examsessionbasic'].$questionid;
	            $this->basic->addQuestionHistory(array('keystring'=>strval($key),'uid'=>intval($sessionvars['examsessionuserid']),'answer'=>$answer,'ctime'=>time()),true);
			    exit;
			break;
			...
			//评分和显示分数
			case 'score':
			    if($this->favor->getExamHistoryNumber($this->_user['sessionuserid'],$this->_user['sessioncurrent'],2)){
			        $message = array(
			            'statusCode' => 300,
			            "message" => "你已经考过试，不能重复考试",
			            "callbackType" => 'forward',
			...
			if(!$sessionvars){
			    header("location:index.php?exam-app-exam-selectquestions");
			}
			
			$lefttime = 0;
			$questype = $this->basic->getQuestypeList();
			if($sessionvars['examsessionstatus'] == 1)
			{
			...
			{
			    //读取考题存库
		        foreach($sessionvars['examsessionquestion']['questions'] as $k => $val){
		            foreach($val as $kk =>$value){
		                $key = $sessionvars['examsessionuserid'].$sessionvars['examsessionbasic'].$value['questionid'];
		                $answerHistory = $this->basic->getQuestionAnswerHistory(strval($key));
		                if(!$answerHistory['answer']){
		                    $this->basic->addQuestionHistory(array('keystring'=>strval($key),'uid'=>intval($sessionvars['examsessionuserid'])));
		                }elseif($answerHistory['ctime']<($sessionvars['examsessionstarttime']+3600)) {
		                    $sessionvars['examsessionquestion']['questions'][$k][$kk]['answerhistory'] = json_decode($answerHistory['answer'],true)[$value['questionid']];
		                }
		            }
		        }
			...
			if(!$r['examid'] && !$this->data['currentbasic']['obbasicid'])
			{
				$message = array(
					'statusCode' => 300,
			...
			{//FIXME
			   /*  $history = $favor = $this->G->make('favor','exam')->getDatabyUidBid($this->_user['sessionuserid'],$this->data['currentbasic']['obbasicid']);
			    if(!empty($history)){
			        $message = array(
			            'statusCode' => 300,
			            "message" => "您已经参加过考试，无法再次参加"
			        );
			        $this->G->R($message);
			    } */
				if($r['examtype'] == 2)
