...
function dTree(objName,iconPath) {
	...
	if(!iconPath)iconPath="../../res/dtree/img/";
	this.icon = {
...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.useRadio && node.id != 0){
            str +='<input type="radio" name="chk" id="r'+  this.obj + nodeId + '"  ondblclick="javascript:getRadioSelected('+nodeId+',\''+node.name+'\')" value="'+node.id+'" />';
			//alert(str);
      }	  
	if (node.url) {
...
dTree.prototype.nodeStatus = function(status, id, bottom) {
	...
	if(eJoin) eJoin.src = (this.config.useLines)?
