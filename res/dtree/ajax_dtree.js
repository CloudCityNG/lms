...
function Node(cfg) {
	...
	this._ai = 0;
	this._p;
	this.hasChild = cfg.hasChild || false;
};

Node.prototype.toString = function() {
	return "[id=" + this.id + ", pid=" + this.pid + ", name=" + this.name
			+ ", url=" + this.url + ", title=" + this.title + ", target="
			+ this.target + ", icon=" + this.icon + ", iconOpen="
			+ this.iconOpen + ", _io=" + this._io + ", _is=" + this._is
			+ ", _ls=" + this._ls + ", _hc=" + this._hc + ", _ld=" + this._ld
			+ ", _ai=" + this._ai + "]";
};

// Tree object
function dTree(objName, oTarget) {
	this.config = {
		target : null,
		folderLinks : true,
		useSelection : true,
		...
		inOrder : false,
		imagePath : '../../res/dtree/img/',
		drawObj : document.body,
		url : ''
	}
	...
	this.icon = {
		...
		loading : this.config.imagePath + '/blue_loading.gif',
		error : this.config.imagePath + '/error.gif'
	};

	this.obj = objName;
	this.oTarget = oTarget || document.body;
	this.aNodes = [];
	this.aIndent = [];
	this.root = new Node({
		id : '-1'
...
dTree.prototype.add = function(cfg) {
	this.aNodes[this.aNodes.length] = new Node(cfg);
};
...
// Cretate the tree to paget
dTree.prototype.render = function() {
	this.config.drawObj.innerHTML = this.toString();
};
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (node._hc) {
		...
		str += (node.pid != '-1' ? this.loading(node) : this.addNode(node));
		str += '</div>';
	}
	this.aIndent.pop();
	return str;
};
// Adds the loading icons and message
dTree.prototype.loading = function(node) {
	var str = '';
	if (this.root.id != node.pid) {
		if (node.pid != '-1') {
		...
		str += '<img src="'
				+ this.icon.loading
				+ '" alt="Loading...." style="width:18px;height:18px;"/> <span>Loading...</span>';
	}
	return str;
};
// Set Indent from adds the empty or line icons
dTree.prototype.setIndent = function(node) {
	if (node.pid == '-1')
		return;
	var _nd = this.aNodes[node.pid];
	_nd._ls ? this.aIndent.push(0) : this.aIndent.push(1);
	// log.debug(_nd.id + ">>>>" +_nd._ls + "==="+ this.aIndent);
	if (_nd.pid == '0') {
		this.aIndent.reverse();
		// log.info("reverse : " + this.aIndent)
	} else if (_nd.pid != '-1') {
		// log.warn("add.."+_nd);
		this.setIndent(_nd);
	}
...
dTree.prototype.setCS = function(node) {
	var lastId;
	node._hc = node.hasChild;
	for (var n = 0; n < this.aNodes.length; n++) {
		if (this.aNodes[n].pid == node.pid)
			lastId = this.aNodes[n].id;
	}
	node._ls = (lastId == node.id);
};
...
dTree.prototype.o = function(id) {
	...
	// show child with ajax
	if (cn._hc && !cn._ld)
		this.show(cn, id);
};
dTree.prototype.show = function(node, id) {
	var eDiv = document.getElementById('d' + this.obj + id);
	var oTree = this;
	var _node = node;
	if (this.config.url) {
		// log.error(node.id + " - " + node.pid + " - " + id)
		var submitUrl = this.config.url + '&pid=' + id ;
		postXmlHttp(submitUrl, function(xh) {
			//alert(xh.responseText);
			eval("var nodes = " + xh.responseText);
			if (nodes) {
			...
			oTree.setCS(node);
			eDiv.innerHTML = oTree.addNode(node);
			_node._ld = true;
		}, function(xh) {
			eDiv.style.display = 'block';
		}, function(xh) {
			var eImgs = eDiv.getElementsByTagName("img");
			eImgs[eImgs.length - 1].src = oTree.icon.error;
			var msgSpan = eDiv.getElementsByTagName("span")[0];
			msgSpan.innerHTML = "Request faild :" + xh.status;
			eDiv.style.display = 'block';
		})
	}
};
...
dTree.prototype.oAll = function(status) {
	for (var n = 0; n < this.aNodes.length; n++) {
		if (this.aNodes[n]._hc && this.aNodes[n]._ld && this.aNodes[n].pid != this.root.id) {
...
// http://localhost:8081/EmisTree/js/emistree.js :: postXmlHttp :: line 593"
// data: no]
var Ajax = {
...
// Get data by ajax
function postXmlHttp(submitUrl, callbackFunc, loadFunc, faildFunc) {
	if (!submitUrl)
		return;
	var _xmlHttpRequestObj = Ajax.getTransport();
	var _postXmlHttpProcessPostChangeCallBack = callbackFunc;
	var _loadingFunction = loadFunc;
	var _loadFaildFunction = faildFunc;
		...
		_xmlHttpRequestObj.onreadystatechange = function() {
			if (_xmlHttpRequestObj.readyState == 4) {
				if (_xmlHttpRequestObj.status == 200) {
					setTimeout(function() {
						_postXmlHttpProcessPostChangeCallBack(_xmlHttpRequestObj);
					}, 500);
				} else {
					setTimeout(function() {
						_loadFaildFunction(_xmlHttpRequestObj);
					}, 500);
				}
			} else if (_xmlHttpRequestObj.readyState == 1) {
				setTimeout(function() {
					_loadingFunction(_xmlHttpRequestObj);
				}, 2);
			}
		};
		_xmlHttpRequestObj.setRequestHeader("Content-Type",
